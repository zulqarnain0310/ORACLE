...
BEGIN
   ...
   IF utils.object_id('TEMPDB..tt_DimUserInfo') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DimUserInfo ';
   END IF;
   DELETE FROM tt_DimUserInfo;
   UTILS.IDENTITY_RESET('tt_DimUserInfo');

   INSERT INTO tt_DimUserInfo SELECT * 
        FROM ( SELECT * ,
                      ROW_NUMBER() OVER ( PARTITION BY UserLoginid ORDER BY EffectiveFromTimeKey DESC, EntityKey DESC  ) RN  
               FROM DimUserInfo  ) A
       WHERE  RN = 1;
   IF utils.object_id('TEMPDB..tt_LogAttendance') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_LogAttendance ';
   END IF;
   DELETE FROM tt_LogAttendance;
   UTILS.IDENTITY_RESET('tt_LogAttendance');

   INSERT INTO tt_LogAttendance SELECT DISTINCT LogCreatedBy UserID  ,
                                                C.UserName ,
                                                Email_ID ,
                                                Designation ,
                                                D.RoleDescription ,
        ...
        FROM SysUserActivityLog_Attendence a
               ...
               JOIN tt_DimUserInfo c   ON a.LogCreatedBy = c.UserLoginID
