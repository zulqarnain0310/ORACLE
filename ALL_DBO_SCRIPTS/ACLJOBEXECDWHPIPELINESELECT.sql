...
BEGIN
   ...
   IF utils.object_id('tempdb..tt_TEMP_DWH') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMP_DWH ';
   END IF;
   IF utils.object_id('tempdb..tt_TEMP_DWH2') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMP_DWH2 ';
   END IF;
   DELETE FROM tt_TEMP_DWH;
   UTILS.IDENTITY_RESET('tt_TEMP_DWH');

   INSERT INTO tt_TEMP_DWH ( 
   ...
   DELETE FROM tt_TEMP_DWH2;
   UTILS.IDENTITY_RESET('tt_TEMP_DWH2');

   INSERT INTO tt_TEMP_DWH2 SELECT ROW_NUMBER() OVER ( PARTITION BY DWHSourcesystems ORDER BY DWHSourcesystems, date_of_data  ) RN_Count  ,
                                   DWHSourcesystems ,
