...
BEGIN
   ...
   BEGIN
      ...
      IF  --SQLDEV: NOT RECOGNIZED
      IF tt_ACCOUNTCAL_hIST  --SQLDEV: NOT RECOGNIZED
      tt_ACCOUNTCAL_hIST TABLE IF  --SQLDEV: NOT RECOGNIZED
      ...
      DELETE FROM tt_CustomerCAL_hIST;
      UTILS.IDENTITY_RESET('tt_CustomerCAL_hIST');

      INSERT INTO tt_CustomerCAL_hIST ( 
      	SELECT * 
            ...
            SELECT * 
              FROM ( SELECT DISTINCT A.CustomerACID ACID  ,
                     ...
                     FROM tt_ACCOUNTCAL_hIST A
                            JOIN tt_CustomerCAL_hIST B   ON A.CustomerEntityId = B.CustomerEntityId
                          --INNER JOIN Accountlevelmoc_mod C ON C.AccountID=A.CustomerAcID

                            LEFT JOIN tt_Temp_24 C   ON C.AccountID = A.CustomerAcID
         ...
         SELECT * 
           FROM ( SELECT A.AccountID ACID  ,
                         ...
                         ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                          FROM DUAL  )  ) RowNumber  
                  FROM AccountLevelMOC_Mod A
                         JOIN tt_ACCOUNTCAL_hIST C   ON C.CustomerAcID = A.AccountID
                         JOIN tt_CustomerCAL_hIST B   ON C.RefCustomerID = B.Refcustomerid
                   WHERE  A.EffectiveFromTimeKey <= v_Timekey
                            AND A.EffectiveToTimeKey >= v_Timekey
