...
BEGIN
   ...
   IF tt_DPD  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_DPD;
   UTILS.IDENTITY_RESET('tt_DPD');
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_DPD 
   ...
   /* CALCULATE MAX DPD */
   IF utils.object_id('TEMPDB..tt_TEMPTABLE') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE ';
   END IF;
   ...
   INSERT INTO ACL_NPA_DATA
     ( SELECT UTILS.CONVERT_TO_NVARCHAR2(SYSDATE,30,p_style=>105) Generation_Date  ,
       ...
       FROM PRO_RBL_MISDB_PROD.CustomerCal_Hist A
              ...
              LEFT JOIN tt_DPD DPD   ON DPD.AccountEntityID = b.AccountEntityID
