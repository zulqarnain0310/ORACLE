...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF tt_tmp21  --SQLDEV: NOT RECOGNIZED
         tt_tmp21 TABLE IF  --SQLDEV: NOT RECOGNIZED
         IF tt_tmp22  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_tmp21;
         UTILS.IDENTITY_RESET('tt_tmp21');

         INSERT INTO tt_tmp21 ( 
         ...
         FROM A ,( SELECT ProvisionAlt_Key ,
                ...
                JOIN tt_tmp21 A   ON A.AccountId = X1.CustomerAcID
         ...
         --Select 'tt_tmp22',D.ProvisionUnSecured,D.ProvisionAlt_Key
         --From tt_tmp21 A 
   ...
   BEGIN
      ...
      BEGIN
         IF tt_tmp  --SQLDEV: NOT RECOGNIZED
         tt_tmp TABLE IF  --SQLDEV: NOT RECOGNIZED
         IF tt_tmp1  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_tmp;
         ...
         FROM A ,( SELECT ProvisionAlt_Key ,
                   ...
                   FROM PRO_RBL_MISDB_PROD.AccountCal_Hist A
                    WHERE  A.EffectiveFromTimeKey <= v_Timekey
                             AND A.EffectiveToTimeKey >= v_Timekey ) X1
                LEFT JOIN tt_tmp A   ON A.AccountId = X1.CustomerAcID
                LEFT JOIN DimProvision_Seg D   ON X1.ProvisionAlt_Key = D.ProvisionAlt_Key ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurrentProvisionPer = src.CurrentProvisionPer;
         OPEN  v_cursor FOR
   ...
   BEGIN
      ...
      BEGIN
         IF tt_tmp11  --SQLDEV: NOT RECOGNIZED
         tt_tmp11 TABLE IF  --SQLDEV: NOT RECOGNIZED
         IF tt_tmp12  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_tmp11;
         UTILS.IDENTITY_RESET('tt_tmp11');

         INSERT INTO tt_tmp11 ( 
         	SELECT A.CustomerACID AccountId  ,
                 SEG.AcBuRevisedSegmentCode SegmentDescription  ,
                 D.AssetClassName ,
                 v_Secured_Unsecured SecuredUnsecured  ,
         ...
         FROM A ,( SELECT ProvisionAlt_Key ,
                   ...
                   FROM PRO_RBL_MISDB_PROD.AccountCal_Hist A
                    WHERE  A.EffectiveFromTimeKey <= v_Timekey
                             AND A.EffectiveToTimeKey >= v_Timekey ) X1
                LEFT JOIN tt_tmp11 A   ON A.AccountId = X1.CustomerAcID
                LEFT JOIN DimProvision_Seg D   ON X1.ProvisionAlt_Key = D.ProvisionAlt_Key ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurrentProvisionPer = src.CurrentProvisionPer;
         OPEN  v_cursor FOR
