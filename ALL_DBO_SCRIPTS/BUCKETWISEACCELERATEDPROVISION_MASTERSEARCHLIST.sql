   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            IF utils.object_id('TempDB..tt_temp_20') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp_20 ';
            END IF;
            IF utils.object_id('TempDB..tt_temp_201') IS NOT NULL THEN
         ...
         ELSE
            ...
            BEGIN
               IF utils.object_id('TempDB..tt_temp_2016') IS NOT NULL THEN
                EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp16_13 ';
               END IF;
               DELETE FROM tt_temp16_13;
               UTILS.IDENTITY_RESET('tt_temp16_13');

               INSERT INTO tt_temp16_13 ( 
               	SELECT A.EntityKey ,
                       A.BucketWiseAcceleratedProvisionEntityID ,
                       A.AcceProDuration ,
                       A.AcceProDurationDescription ,
               ...
               MERGE INTO A 
               USING (SELECT A.ROWID row_id, A.SegmentName
               FROM A ,tt_temp16_13 A ) src
               ON ( A.ROWID = src.row_id )
               WHEN MATCHED THEN UPDATE SET A.segmentDescription = src.SegmentName;
               --INNER JOIN DimAcBuSegment C ON C.AcBuSegmentAlt_Key=A.SegmentName
               MERGE INTO A 
               USING (SELECT A.ROWID row_id, C.AssetClassName
               FROM A ,tt_temp16_13 A
                      JOIN DimAssetClass C   ON A.AssetClassNameAlt_key = C.AssetClassAlt_Key ) src
               ON ( A.ROWID = src.row_id )
               WHEN MATCHED THEN UPDATE SET A.AssetClassName = src.AssetClassName;
               MERGE INTO A 
         ...
         BEGIN
            IF utils.object_id('TempDB..tt_temp_2020') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_temp20_13 ';
            END IF;
            DELETE FROM tt_temp20_13;
            UTILS.IDENTITY_RESET('tt_temp20_13');

            INSERT INTO tt_temp20_13 ( 
            	SELECT A.EntityKey ,
                    A.BucketWiseAcceleratedProvisionEntityID ,
                    A.AcceProDuration ,
                    A.AcceProDurationDescription ,
            ...
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, A.SegmentName
            FROM A ,tt_temp20_13 A ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.segmentDescription = src.SegmentName;
            --INNER JOIN DimAcBuSegment C ON C.AcBuSegmentAlt_Key=A.SegmentName
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.AssetClassName
            FROM A ,tt_temp20_13 A
                   JOIN DimAssetClass C   ON A.AssetClassNameAlt_key = C.AssetClassAlt_Key ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.AssetClassName = src.AssetClassName;
            MERGE INTO A 
