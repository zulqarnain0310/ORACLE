--------------------------------------------------------
--  DDL for Procedure INVESTMENTDATAPROCESSING_10122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" 
----/*=========================================
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         USING (SELECT A.ROWID row_id, B.FinalAssetClassAlt_Key, B.NPIDt
         FROM A ,CurDat_RBL_MISDB_PROD.InvestmentFinancialDetail A
                JOIN InvestmentFinancialDetail B   ON SUBSTR(A.RefInvID, 1, INSTR(A.RefInvID, '_') - 1) = B.RefInvID
                AND A.EffectiveFromTimeKey <= v_timekey
                AND A.EffectiveToTimeKey >= v_timekey
                AND B.EffectiveFromTimeKey <= v_timekey - 1
                AND B.EffectiveToTimeKey >= v_timekey - 1 ) src
         ...
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET Cc.Asset_Norm = 'ALWYS_NPA';
         ...
         ELSE A.NPIDt
            END AS pos_4
         FROM A ,RBL_MISDB_PROD.InvestmentFinancialDetail A
         ...
         ELSE A.NPIDt
            END AS pos_4
         FROM A ,RBL_MISDB_PROD.InvestmentFinancialDetail A
                JOIN RBL_MISDB_PROD.InvestmentIssuerDetail B   ON A.RefIssuerID = B.IssuerID
                AND A.EffectiveFromTimeKey <= v_TIMEKEY
                AND A.EffectiveToTimeKey >= v_TIMEKEY
         ...
         IF tt_HISTORYDATA_4  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_HISTORYDATA_4;
         UTILS.IDENTITY_RESET('tt_HISTORYDATA_4');

         INSERT INTO tt_HISTORYDATA_4 ( 
         	SELECT DISTINCT A.ACID ,
                          A.StatusType ,
                          A.StatusDate ,
                          MAX(A.EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM RBL_MISDB_PROD.ExceptionFinalStatusType A
                   LEFT JOIN RBL_MISDB_PROD.InvestmentIssuerDetail B   ON A.ACID = B.IssuerID
                   AND B.EffectiveFromTimeKey <= v_TIMEKEY
                   AND B.EffectiveToTimeKey >= v_TIMEKEY
         	...
         	SELECT DISTINCT A.ACID ,
                          ...
                          MAX(A.EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM RBL_MISDB_PROD.ExceptionFinalStatusType A
                   LEFT JOIN RBL_MISDB_PROD.InvestmentFinancialDetail B   ON A.ACID = B.RefInvID
                   AND B.EffectiveFromTimeKey <= v_TIMEKEY
                   AND B.EffectiveToTimeKey >= v_TIMEKEY
         	 WHERE  B.RefInvID IS NULL
         	  ...
         	  GROUP BY A.ACID,A.StatusType,A.StatusDate );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_HISTORYDATA_41  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_HISTORYDATA1_4;
         UTILS.IDENTITY_RESET('tt_HISTORYDATA1_4');

         INSERT INTO tt_HISTORYDATA1_4 ( 
         	SELECT A.UcifId ,
                 MIN(B.StatusDate)  StatusDate  ,
                 B.StatusType 
         	  FROM RBL_MISDB_PROD.InvestmentIssuerDetail A
                   JOIN tt_HISTORYDATA_4 B   ON A.IssuerID = B.ACID
         	...
         	SELECT C.UcifId ,
         	  ...
         	  FROM RBL_MISDB_PROD.InvestmentFinancialDetail A
                   JOIN RBL_MISDB_PROD.InvestmentIssuerDetail C   ON A.RefIssuerID = C.IssuerID
                   JOIN tt_HISTORYDATA_4 B   ON A.RefInvID = B.ACID
                   AND A.EffectiveFromTimeKey <= B.EffectiveFromTimeKey
                   AND A.EffectiveToTimeKey >= B.EffectiveFromTimeKeY
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_HISTORYDATA_42  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_HISTORYDATA2_4;
         UTILS.IDENTITY_RESET('tt_HISTORYDATA2_4');

         INSERT INTO tt_HISTORYDATA2_4 ( 
         	SELECT A.UcifId ,
                 B.ACID ,
                 B.StatusType 
         	  FROM RBL_MISDB_PROD.InvestmentIssuerDetail A
         ...
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, CONCAT(DEGREASON, ',NPA DUE TO Linkage By Issuerid=', D.ACID) AS pos_2, CASE 
         ...
         FROM A ,InvestmentFinancialDetail A
                ...
                AND B.EffectiveFromTimeKey <= v_TIMEKEY
                AND B.EffectiveToTimeKey >= v_TIMEKEY
                JOIN tt_HISTORYDATA1_4 C   ON B.UcifId = C.UcifId
                AND C.StatusType = 'Settlement'
                LEFT JOIN tt_HISTORYDATA2_4 D   ON C.UcifId = D.UcifId 
          WHERE A.FINALASSETCLASSALT_KEY > 1) src
         ON ( A.ROWID = src.row_id )
         ...
         WHEN MATCHED THEN UPDATE SET A.FLGDEG = 'Y',
                                      ...
                                      A.NPIDt = pos_4;
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, CONCAT(DEGREASON, ',NPA DUE TO Linkage By Invid=', D.ACID) AS pos_2, CASE 
         WHEN C.StatusDate < A.NPIDt THEN C.StatusDate
         ELSE A.NPIDt
            END AS pos_3
         FROM A ,InvestmentFinancialDetail A
         ...
         */
         ...
         ----IF OBJECT_ID('TEMPDB..tt_TEMPMINASSETCLASS_10') IS NOT NULL
         ----  DROP TABLE tt_TEMPMINASSETCLASS_10

         ----	SELECT UcifId,MAX(ISNULL(FinalAssetClassAlt_Key,1)) FinalAssetClassAlt_Key
         ----	,MIN(NPIDt) NPIDt 
         ----	 INTO tt_TEMPMINASSETCLASS_10 
         ...
         ----	  ALTER TABLE  tt_TEMPMINASSETCLASS_10 ADD RefInvID VARCHAR (200)


         ----	  UPDATE A SET RefInvID=B.RefInvID
         ----	 FROM tt_TEMPMINASSETCLASS_10  A
         ...
         */
         IF utils.object_id('TEMPDB..tt_TEMPMINASSETCLASS_10') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPMINASSETCLASS_10 ';
         END IF;
         DELETE FROM tt_TEMPMINASSETCLASS_10;
         UTILS.IDENTITY_RESET('tt_TEMPMINASSETCLASS_10');

         INSERT INTO tt_TEMPMINASSETCLASS_10 ( 
         	SELECT UcifId ,
         ...
         FROM D ,tt_TEMPMINASSETCLASS_10 A
         ...
         ----------------------------------------------------------------
         --- SELECT * FROM tt_TEMPMINASSETCLASS_10  
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_ADHOC_UCICID_STD_4  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_ADHOC_UCICID_STD_4;
         UTILS.IDENTITY_RESET('tt_ADHOC_UCICID_STD_4');

         INSERT INTO tt_ADHOC_UCICID_STD_4 ( 
         	SELECT UCICID 
         	  FROM CalypsoInvMOC_ChangeDetails 
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                    AND EFFECTIVETOTIMEKEY >= v_TIMEKEY
                    AND AssetClassAlt_Key = 1 );
         WITH CTE_A AS ( SELECT IssuerID 
           FROM tt_ADHOC_UCICID_STD_4 A
                  JOIN InvestmentIssuerDetail B   ON A.UCICID = B.UcifId
           GROUP BY IssuerID ) 
            MERGE INTO C 
            USING (SELECT C.ROWID row_id, v_TIMEKEY
            FROM C ,CTE_A A
                   JOIN ExceptionFinalStatusType C   ON A.IssuerID = C.ACID
                   AND C.EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                   AND C.EFFECTIVETOTIMEKEY >= v_TIMEKEY
                   AND C.StatusType IN ( 'Settlement' )
            ...
            ;
         WITH CTE_B AS ( SELECT RefInvID 
           FROM tt_ADHOC_UCICID_STD_4 A
                  JOIN InvestmentIssuerDetail B   ON A.UCICID = B.UcifId
                  JOIN InvestmentFinancialDetail C   ON B.IssuerID = C.RefIssuerID
           GROUP BY RefInvID ) 
            MERGE INTO C 
            USING (SELECT C.ROWID row_id, v_TIMEKEY
            FROM C ,CTE_B A
                   JOIN ExceptionFinalStatusType C   ON A.RefInvID = C.ACID
                   AND C.EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                   AND C.EFFECTIVETOTIMEKEY >= v_TIMEKEY
         ...
         ELSE D.Asset_Norm
            END AS pos_5
         FROM D ,CalypsoInvMOC_ChangeDetails A
                JOIN InvestmentIssuerDetail B   ON A.CustomerEntityID = B.IssuerEntityId
                AND B.EffectiveFromTimeKey <= v_TIMEKEY
                AND B.EffectiveToTimeKey >= v_TIMEKEY
                AND A.EffectiveFromTimeKey <= v_TIMEKEY
                AND A.EffectiveToTimeKey >= v_TIMEKEY
                JOIN InvestmentBasicDetail C   ON C.IssuerEntityId = B.IssuerEntityId
                AND C.EffectiveFromTimeKey <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY
                JOIN InvestmentFinancialDetail D   ON C.InvEntityid = D.InvEntityid
         ...
         WHEN MATCHED THEN UPDATE SET D.FinalAssetClassAlt_Key = src.AssetClassAlt_Key,
                                      ...
                                      D.Asset_Norm = pos_5;
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, NULL, 'N', NULL, NULL, 'N'
         FROM A ,InvestmentFinancialDetail A 
          WHERE A.EffectiveFromTimeKey <= v_timekey
           AND A.EffectiveToTimeKey >= v_timekey
           AND FinalAssetClassAlt_Key = 1) src
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INVESTMENTDATAPROCESSING_10122023" TO "ADF_CDR_RBL_STGDB";
