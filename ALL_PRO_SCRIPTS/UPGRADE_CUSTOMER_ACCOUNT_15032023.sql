--------------------------------------------------------
--  DDL for Procedure UPGRADE_CUSTOMER_ACCOUNT_15032023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" /*=========================================
   ...
   BEGIN
      ...
      BEGIN
         ...
         UPDATE PRO_RBL_MISDB_PROD.CUSTOMERCAL
            SET FLGUPG = 'N';
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_170') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_170 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_170;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_170');

         INSERT INTO tt_TEMPTABLE_170 ( 
         	SELECT A.UCIF_ID ,
                 TOTALCOUNT 
                   ...
                   JOIN ( SELECT A.UCIF_ID ,
                          ...
                          FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                           ...
                           WHERE  ( B.DPD_IntService <= B.RefPeriodIntServiceUPG
                                    ...
                                    AND NVL(IntOverdue, 0) = 0 )
                                    OR ( NVL(FacilityType, ' ') NOT IN ( 'CC','OD' )
                                   ) )
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_TEMPTABLE_170 B   ON A.UCIF_ID = B.UCIF_ID
         ...
         /*
         	--  UPGRADE ACCOUNTS- NPA DUTE TO DPD_SELLER/VIRTUAL DPD --
         		DROP TABLE IF EXISTS #NPA_SELLER
         ...
         -------------------
         	--  UPGRADE ACCOUNTS- NPA DUTE TO VIRTUAL DPD - AND VERTIUAL DPD IS 0 --
         		...
         		FROM PRO.ACCOUNTCAL A
         			...
         			WHERE E.NPA_FLAG='NPA_VDPD' and d.PeakDPD is not null
         							AND (ISNULL(D.PeakDPD,0)<A.RefPeriodOverdue and isnull(D.DPD_Seller,0)>0)

         				--AND ISNULL(A.ASSET_NORM,'NORMAL')<>'ALWYS_NPA' 
         		GROUP BY UcifEntityID
         		----select * from #NPA_VDPD where ucifentityid=2860237

         		DELETE A FROM #NPA_VDPD A
         		INNER JOIN  PRO.ACCOUNTCAL B
         			ON A.UcifEntityID =B.UcifEntityID
         	...
         	--  UPGRADE ACCOUNTS- NPA DUTE TO VIRTUAL DPD - AND VERTIUAL DPD IS >0 --
         		DROP TABLE IF EXISTS #NPA_VDPD_1
         		SELECT UcifEntityID
         		...
         		INNER JOIN  PRO.ACCOUNTCAL B
         			ON A.UcifEntityID =B.UcifEntityID
         		WHERE (	isnull(b.dpd_max,0)>0)

         		UPDATE A
         			SET A.FlgUpg='U'
         		FROM PRO.CUSTOMERCAL A
         		 INNER JOIN #NPA_VDPD_1 B
         			ON A.UcifEntityID=B.UcifEntityID
         			----and a.CustomerEntityID=1194338
         		 INNER JOIN DIMASSETCLASS C ON C.AssetClassAlt_Key=A.SYSASSETCLASSALT_KEY 
         			AND (C.EffectiveFromTimeKey<=@TIMEKEY AND C.EffectiveToTimeKey>=@TIMEKEY)
         		...
         		-- SET NPA_FLAG AS NULL FOR STD ACCOUNTS --
         			UPDATE d
         				SET NPA_FLAG=NULL
         			FROM PRO.ACCOUNTCAL A
         			INNER JOIN PRO.BuyoutUploadDetailsCal D
         ...
         */
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_1701') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE1_10 ';
         END IF;
         DELETE FROM tt_TEMPTABLE1_10;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE1_10');

         INSERT INTO tt_TEMPTABLE1_10 ( 
         	SELECT A.UCIF_ID ,
                 TOTALCOUNT 
         	  FROM ( SELECT A.UCIF_ID ,
                          COUNT(1)  TOTALCOUNT  
         ...
         USING (SELECT A.ROWID row_id, 'U'
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_TEMPTABLE1_10 B   ON A.UCIF_ID = B.UCIF_ID
                JOIN DIMASSETCLASS C   ON C.AssetClassAlt_Key = A.SysAssetClassAlt_Key
                AND ( C.EffectiveFromTimeKey <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY ) 
          WHERE ( NOT ( NVL(A.Asset_Norm, 'NORMAL') = 'ALWYS_NPA' )
         ...
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.FlgUpg = 'U';
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_170RefCustomerID') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLERefCustomerID_5 ';
         END IF;
         DELETE FROM tt_TEMPTABLERefCustomerID_5;
         UTILS.IDENTITY_RESET('tt_TEMPTABLERefCustomerID_5');

         INSERT INTO tt_TEMPTABLERefCustomerID_5 ( 
         ...
         USING (SELECT A.ROWID row_id, 'U'
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_TEMPTABLERefCustomerID_5 B   ON A.RefCustomerID = B.RefCustomerID
                JOIN DIMASSETCLASS C   ON C.AssetClassAlt_Key = A.SysAssetClassAlt_Key
                AND ( C.EffectiveFromTimeKey <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY ) 
          WHERE ( NOT ( NVL(A.Asset_Norm, 'NORMAL') = 'ALWYS_NPA' )
         ...
         IF utils.object_id('TEMPDB..tt_PANUPDATEUPGRADE_5') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PANUPDATEUPGRADE_5 ';
         END IF;
         DELETE FROM tt_PANUPDATEUPGRADE_5;
         UTILS.IDENTITY_RESET('tt_PANUPDATEUPGRADE_5');

         INSERT INTO tt_PANUPDATEUPGRADE_5 ( 
         	SELECT A.PANNO ,
                 A.TotalCountMAX ,
                 B.TotalCount 
         ...
         FROM B ,tt_PANUPDATEUPGRADE_5 A
         ...
         FROM A ,PRO_RBL_MISDB_PROD.AdvAcRestructureCal A
          ...
          WHERE D.DimParameterName = 'TypeofRestructuring'
           ...
           AND C.FlgUpg = 'U') src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET C.FLGUPG ----- DPD_Breach_Date IS NOT NULL  and commented by Amar - As disxussed with Ashish Sir on 27/04/2022 at 15:17 time for account should not upgrade in any case with specified period
                                       -- AS DISCUSSED WITH aSHISH sIR - PRIDENTIAL ACCOUNTS NEVER WILL WILL UPGRADE- WILL BE NPA TILL THEN CLOSED
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."UPGRADE_CUSTOMER_ACCOUNT_15032023" TO "ADF_CDR_RBL_STGDB";
