--------------------------------------------------------
--  DDL for Procedure MARKING_FLGPNPA_21082024
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."MARKING_FLGPNPA_21082024" /*=========================================
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         FROM A ,tt_CUSTOMERCAL_44 A ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.FlgPNPA = 'N',
                                      PNPA_Dt = NULL,
                                      PNPA_Class_Key = NULL;
         ...
         FROM A ,tt_AccountCal_33 A
                JOIN tt_CUSTOMERCAL_44 B   ON A.RefCustomerID = B.RefCustomerID 
         ...
         DELETE FROM tt_PNPA_REASON_DATE_4;
         ...
         IF tt_PNPA_REASON_DATE_4  --SQLDEV: NOT RECOGNIZED
         INSERT INTO tt_PNPA_REASON_DATE_4
         ...
         */
         ...
         FROM B ,tt_AccountCal_33 A
                JOIN tt_CUSTOMERCAL_44 B   ON A.CustomerEntityID = B.CustomerEntityID 
          WHERE A.FlgPNPA = 'Y'
           AND ( B.FlgProcessing = 'N' )) src
         ON ( B.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET B.FlgPNPA = 'Y';
         IF utils.object_id('TEMPDB..tt_TEMPTABLEPNPA_5') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPNPA_5 ';
         END IF;
         DELETE FROM tt_TEMPTABLEPNPA_5;
         UTILS.IDENTITY_RESET('tt_TEMPTABLEPNPA_5');

         INSERT INTO tt_TEMPTABLEPNPA_5 ( 
         ...
         FROM A ,tt_CUSTOMERCAL_44 A
                JOIN tt_TEMPTABLEPNPA_5 B   ON A.CustomerEntityID = B.CustomerEntityID ) src
         ON ( A.ROWID = src.row_id )
         ...
         USING (SELECT B.ROWID row_id, 'Y'
         FROM B ,tt_AccountCal_33 A
                JOIN tt_CUSTOMERCAL_44 B   ON A.UcifEntityID = B.UcifEntityID 
          WHERE A.FlgPNPA = 'Y'
           AND ( B.FlgProcessing = 'N' )) src
         ON ( B.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET B.FlgPNPA = 'Y';
         IF utils.object_id('TEMPDB..tt_CTE_CUSTOMERWISEBALANCEP_5') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CTE_CUSTOMERWISEBALANCEP_5 ';
         END IF;
         DELETE FROM tt_CTE_CUSTOMERWISEBALANCEP_5;
         UTILS.IDENTITY_RESET('tt_CTE_CUSTOMERWISEBALANCEP_5');

         INSERT INTO tt_CTE_CUSTOMERWISEBALANCEP_5 ( 
         ...
         USING (SELECT A.ROWID row_id, B.PNPA_DATE, 'Y', b.PNPA_Class_Key
         FROM A ,tt_CUSTOMERCAL_44 A
                JOIN tt_CTE_CUSTOMERWISEBALANCEP_5 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.PNPA_DT = src.PNPA_DATE,
                                      A.FlgPNPA = 'Y',
                                      a.PNPA_Class_Key = src.PNPA_Class_Key;
         ...
         USING (SELECT a.ROWID row_id, b.PNPA_DATE, 'Y', b.PNPA_Class_Key
         FROM a ,tt_AccountCal_33 a
                JOIN tt_CTE_CUSTOMERWISEBALANCEP_5 b   ON a.UcifEntityID = b.UcifEntityID
                AND ( NVL(A.Asset_Norm, 'NORMAL') <> 'ALWYS_STD' ) ) src
         ON ( a.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET a.PNPA_DATE = src.PNPA_DATE,
                                      a.FlgPNPA = 'Y',
         ...
         FROM A ,tt_AccountCal_33 a
                JOIN tt_AccountCal_33 b   ON a.UcifEntityID = b.UcifEntityID 
          WHERE b.FlgPNPA = 'Y'
           AND A.FlgPNPA = 'Y'
           AND A.PNPA_Reason IS NULL
           AND B.PNPA_Reason IS NOT NULL
         ...
         ------IF OBJECT_ID('TEMPDB..tt_CTE_CUSTOMERWISEBALANCEP_5') IS NOT NULL
         ------   DROP TABLE tt_CTE_CUSTOMERWISEBALANCEP_5
         ------SELECT A.UcifEntityID,SUM(ISNULL(A.BALANCE,0)) BALANCE INTO tt_CTE_CUSTOMERWISEBALANCEP_5
         ------ FROM PRO.ACCOUNTCAL A 
         ------ INNER JOIN PRO.CUSTOMERCAL B ON A.UcifEntityID=B.UcifEntityID
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."MARKING_FLGPNPA_21082024" TO "ADF_CDR_RBL_STGDB";
