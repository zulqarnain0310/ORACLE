--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_22112023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" /*=========================================  
      ...
      BEGIN
         ...
         ( SELECT A.ROWID
           FROM PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                  LEFT JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL B   ON A.CustomerEntityID = B.CustomerEntityID,
                A
                --select count(1)  
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_21') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_21 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_21;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_21');

            INSERT INTO tt_TEMPTABLEPANCARD_21 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_21 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_21 A
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_21 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_21 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_21') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_21 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_21;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_21');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_21 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_21 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_21 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         ELSE SysNPA_Dt
            END AS pos_4
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL   ON A.CustomerEntityID = ACL.CustomerEntityID
                JOIN ExceptionFinalStatusType C   ON ACL.CustomerAcID = C.ACID
                AND NVL(c.SourceAlt_Key, 0) <> 7
                AND C.EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY 
         ...
         ------DROP TABLE IF EXISTS #ALWYS1
         ------SELECT DISTINCT A.UCIF_ID,C.StatusType,MIN(C.StatusDate)StatusDate INTO #ALWYS1
         ------FROM PRO.CUSTOMERCAL A
         ------INNER JOIN PRO.ACCOUNTCAL B
         ------ON A.RefCustomerID=B.RefCustomerID
         ------INNER JOIN  ExceptionFinalStatusType C   
         ...
         IF tt_excepdata_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_excepdata_10;
         UTILS.IDENTITY_RESET('tt_excepdata_10');

         INSERT INTO tt_excepdata_10 ( 
         	SELECT acid ,
         	 ...
         	 WHERE  StatusType IN ( 'TWO','WO','Charge Off','Settlement' )
         	  ...
         	  GROUP BY ACID );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPACID_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPACID_10;
         UTILS.IDENTITY_RESET('tt_EXPACID_10');

         INSERT INTO tt_EXPACID_10 ( 
         	SELECT a.acid ,
                 StatusType ,
                 UTILS.CONVERT_TO_NUMBER(0,19,0) CUSTOMERENTITYID  ,
                 UTILS.CONVERT_TO_NUMBER(0,19,0) UCIFENTITYID  
         	  FROM ExceptionFinalStatusType a
                   JOIN tt_excepdata_10 b   ON a.acid = b.acid
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_ADVACID_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_ADVACID_10;
         UTILS.IDENTITY_RESET('tt_ADVACID_10');

         INSERT INTO tt_ADVACID_10 ( 
         	SELECT AccountEntityId ,
                 MAX(EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM AdvAcBasicDetail A
                   JOIN tt_EXPACID_10 B   ON A.CustomerACID = B.ACID
         	  GROUP BY AccountEntityId );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPCUST_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPCUST_10;
         UTILS.IDENTITY_RESET('tt_EXPCUST_10');

         INSERT INTO tt_EXPCUST_10 ( 
         	SELECT CustomerEntityId ,
                 CustomerACID 
         	  FROM AdvAcBasicDetail A
                   JOIN tt_ADVACID_10 B   ON A.AccountEntityId = B.AccountEntityId
                   AND a.EffectiveFromTimeKey = B.EffectiveFromTimeKey );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPCUST_10ID  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPCUSTID_10;
         UTILS.IDENTITY_RESET('tt_EXPCUSTID_10');

         INSERT INTO tt_EXPCUSTID_10 ( 
         	SELECT A.CustomerEntityId ,
                 MAX(EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM CustomerBasicDetail A
                   JOIN tt_EXPCUST_10 B   ON A.CustomerEntityId = B.CustomerEntityId
         	  GROUP BY A.CustomerEntityId );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPUCIC_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPUCIC_10;
         UTILS.IDENTITY_RESET('tt_EXPUCIC_10');

         INSERT INTO tt_EXPUCIC_10 ( 
         	SELECT A.CustomerEntityId ,
                 UcifEntityID 
         	  FROM CustomerBasicDetail A
                   JOIN tt_EXPCUSTID_10 B   ON A.CustomerEntityId = B.CustomerEntityId
                   AND a.EffectiveFromTimeKey = B.EffectiveFromTimeKey );
         ---------------
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, B.CustomerEntityId
         FROM A ,tt_EXPACID_10 A
                JOIN tt_EXPCUST_10 B   ON A.ACID = B.CustomerACID ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,tt_EXPACID_10 A
                JOIN tt_EXPUCIC_10 B   ON A.CUSTOMERENTITYID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.UCIFENTITYID = src.UCIFENTITYID;
         DELETE tt_EXPACID_10

          WHERE  UCIFENTITYID = 0;
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_LINKACEXP_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_LINKACEXP_10;
         UTILS.IDENTITY_RESET('tt_LINKACEXP_10');

         INSERT INTO tt_LINKACEXP_10 ( 
         	SELECT a.UCIFENTITYID ,
                 STRING_AGG(ACID, '|') ACID  
         	  FROM tt_EXPACID_10 a
                   LEFT JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL b   ON a.acid = b.CustomerAcID
         ...
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL
                JOIN tt_EXPACID_10 A   ON ACL.UcifEntityID = A.UcifEntityID
                JOIN tt_LINKACEXP_10 b   ON a.UCIFENTITYID = acl.UcifEntityID 
         ...
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL
                JOIN tt_EXPACID_10 A   ON ACL.UcifEntityID = A.UcifEntityID
                JOIN tt_LINKACEXP_10 b   ON a.UCIFENTITYID = B.UcifEntityID 
         ...
         MERGE INTO ACL 
         USING (SELECT ACL.ROWID row_id, 'ALWYS_NPA'
         FROM ACL ,PRO_RBL_MISDB_PROD.CUSTOMERCAL ACL
                JOIN tt_EXPACID_10 A   ON ACL.UcifEntityID = A.UcifEntityID 
          WHERE A.StatusType = 'Settlement'
           AND SysAssetClassAlt_Key > 1) src
         ON ( ACL.ROWID = src.row_id )
         ...
         --IF OBJECT_ID('TEMPDB..tt_PRVQTRRV_21') IS NOT NULL  
         --DROP TABLE tt_PRVQTRRV_21  
         ...
         --FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_21 B  
         ...
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_21') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_21 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_21;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_21');

         INSERT INTO tt_PRVQTRRV_21 ( 
         	SELECT A.CustomerEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_21 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --UPDATE A SET A.PRVQTRRV= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)  FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_21 B                  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_21') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_21 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_21;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_21');

         INSERT INTO tt_CurntQtrRv_21 ( 
         	SELECT CustomerEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_21 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key <> 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = CurrentValue;
         ------- ----Added By Mandeep (Security Perfection)
         DELETE FROM tt_CurntQtrRv_Finacle_16;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_Finacle_16');

         INSERT INTO tt_CurntQtrRv_Finacle_16 ( 
         	SELECT CustomerEntityId ,
                 SUM(b.CurrentValue)  CurrentValue  
         	  FROM AdvSecurityDetail A
                   JOIN AdvSecurityValueDetail B   ON A.SecurityEntityID = B.SecurityEntityID
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_Finacle_16 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key = 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = src.CurrentValue;
         /*                  
         --IF OBJECT_ID('TEMPDB..tt_CurntQtrRv_21') IS NOT NULL                  
         --DROP TABLE tt_CurntQtrRv_21                  

         --SELECT *                    
         --INTO tt_CurntQtrRv_21                   
         --FROM dbo.AdvCustSecurityFun(@TIMEKEY)                  

         ------UPDATE A SET A.CurntQtrRv= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)                   
         ------FROM PRO.CUSTOMERCAL A INNER  JOIN tt_CurntQtrRv_21 B                  
         ------ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         */
         ...
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A 
         ...
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = NULL,
                                      ...
                                      OverdueAmt = 0,
                                      OtherOverdue = 0;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_21') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_21 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_21;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_21');

         INSERT INTO tt_BILL_OVERDUE_21 ( 
         	SELECT AccountEntityID ,
                 BILLENTITYID ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_21_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_FINAL_20 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_FINAL_20;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_FINAL_20');

         INSERT INTO tt_BILL_OVERDUE_FINAL_20 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  BILOVERDUE  ,
                 MIN(BILLDUEDT)  BILLOVERDUEDT  
         	  FROM tt_BILL_OVERDUE_21 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.BILLOVERDUEDT, B.BILOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_FINAL_20 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET OVERDUESINCEDT = src.BILLOVERDUEDT,
                                      OverdueAmt = src.BILOVERDUE;
         --=====SCF SOURCE SYSTEM CHANGES==================================================================================================
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_21_SCF') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_18 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_18;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_18');

         INSERT INTO tt_BILL_OVERDUE_SCF_18 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
         	  ...
         	  GROUP BY AccountEntityId );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_21_SCF_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_FINAL_18 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_FINAL_18;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_FINAL_18');

         INSERT INTO tt_BILL_OVERDUE_SCF_FINAL_18 ( 
         	SELECT AccountEntityId ,
                 Balance ,
                 overdueinterest ,
                 ...
                 InterestOverdueDate ,
                 BillDueDt 
         	  FROM tt_BILL_OVERDUE_SCF_18  );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.BillDueDt, B.InterestOverdueDate, B.OverDueSinceDt, NVL(B.Balance, 0) AS pos_5, NVL(B.Balance, 0) - NVL(B.OverdueInterest, 0) AS pos_6, NVL(B.OverdueInterest, 0) AS pos_7
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_SCF_FINAL_18 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = src.BillDueDt,
                                      ...
                                      PrincOverdue = pos_6,
                                      IntOverdue = pos_7;
         IF utils.object_id('TEMPDB..tt_BILL_Min_ReviewDt_18') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_Min_ReviewDt_18 ';
         END IF;
         DELETE FROM tt_BILL_Min_ReviewDt_18;
         UTILS.IDENTITY_RESET('tt_BILL_Min_ReviewDt_18');

         INSERT INTO tt_BILL_Min_ReviewDt_18 ( 
         	SELECT AccountEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_Min_ReviewDt_18 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.ReviewDueDt = ReviewDuedate;
         --=====================================================SCF BILL END===========================================================================
         --IF OBJECT_ID('TEMPDB..tt_BILL_OVERDUE_21_FINAL') IS NOT NULL  
         --    DROP TABLE tt_BILL_OVERDUE_21_FINAL  
         --SELECT AccountEntityId,SUM(BALANCE) BILOVERDUE,MIN(BILLDUEDT) BILLOVERDUEDT  
         -- INTO tt_BILL_OVERDUE_21_FINAL  
         --FROM tt_BILL_OVERDUE_21 GROUP BY AccountEntityId  
         --UPDATE PRO.ACCOUNTCAL SET OVERDUESINCEDT=B.BILLOVERDUEDT ,OverdueAmt  =B.BILOVERDUE  
         --FROM  PRO.ACCOUNTCAL A   
         --INNER JOIN tt_BILL_OVERDUE_21_FINAL B  
         -- ON A.AccountEntityID =B.AccountEntityId  
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_21') IS NOT NULL THEN
          --update a  
         ...
         /* START OF PUI WORK*/
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_21 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_21;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_21');

         INSERT INTO tt_PC_OVERDUE_21 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ----- )<=@PROCESSINGDATE  --  as discussed with Triloki Sir for SMA KIssue - Consider 1 dpd on due date  
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_21_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_21 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_21;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_21');

         INSERT INTO tt_PC_OVERDUE_FINAL_21 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_21 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_21 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         ------------END  
         -----------------------------------------------------------------------------------------------------------------  
         --DROP TABLE IF EXISTS tt_CCOD_90DAYS_INTT_CR_AMT_18  
         --SELECT A.CustomerAcID,  
         ...
         -- INTO tt_CCOD_90DAYS_INTT_CR_AMT_18  
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_CCOD_90DAYS_INTT_CR_AMT_18  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CCOD_90DAYS_INTT_CR_AMT_18;
         UTILS.IDENTITY_RESET('tt_CCOD_90DAYS_INTT_CR_AMT_18');

         INSERT INTO tt_CCOD_90DAYS_INTT_CR_AMT_18 ( 
         	SELECT A.CustomerAcID ,
                 SUM(CASE 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL a
                LEFT JOIN tt_CCOD_90DAYS_INTT_CR_AMT_18 b   ON A.CustomerAcID = b.CustomerAcID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_21') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_21 ';
         END IF;
         DELETE FROM tt_Stock_21;
         UTILS.IDENTITY_RESET('tt_Stock_21');

         INSERT INTO tt_Stock_21 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_21_IX ON tt_Stock_21(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_21NON ON tt_Stock_21(AccountEntityId,StkSmtDt)  
         ----IF OBJECT_ID('Tempdb..tt_Stock_212') IS NOT NULL  
         ----DROP TABLE tt_Stock_212  
         ...
         ----   GROUP BY AccountEntityId  
         ----CREATE CLUSTERED INDEX tt_Stock_212_IX ON tt_Stock_212(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_212NON ON tt_Stock_212(AccountEntityId,StkSmtDt)  
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A   
         --INNER  JOIN tt_Stock_21 SD ON A.AccountEntityId=SD.AccountEntityId  
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_41') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_41 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_41;
         INSERT INTO tt_TEMPDerecognisedInterest_41
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_41 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_42') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_42 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_42;
         INSERT INTO tt_TEMPDerecognisedInterest_42
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_42 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         WITH CTE_FDOD_SEC AS ( SELECT C.AccountEntityID ,
           ...
           FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL C
         ...
         FROM C ,PRO_RBL_MISDB_PROD.ACCOUNTCAL C
                JOIN DimProduct DP   ON c.ProductAlt_Key = dp.ProductAlt_Key
                AND DP.ProductGroup = 'FDSEC'
                AND DP.EffectiveFromTimeKey <= v_TimeKey
                AND DP.EffectiveToTimeKey >= v_TimeKey
         ...
         ELSE C.DegReason
            END AS pos_5
         FROM C ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN PRO_RBL_MISDB_PROD.BuyoutUploadDetailsCal B   ON A.CustomerAcID = B.CustomerAcID
                JOIN PRO_RBL_MISDB_PROD.CUSTOMERCAL C   ON A.CustomerEntityID = C.CustomerEntityID ) src
         ON ( C.ROWID = src.row_id )
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_ADHOC_UcifEntityID_STD_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_ADHOC_UcifEntityID_STD_10;
         UTILS.IDENTITY_RESET('tt_ADHOC_UcifEntityID_STD_10');

         INSERT INTO tt_ADHOC_UcifEntityID_STD_10 ( 
         	SELECT UcifEntityID 
         ...
         WITH CTE_A AS ( SELECT CUSTOMERENTITYID 
           FROM tt_ADHOC_UcifEntityID_STD_10 A
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_MOC_DATA_20  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_20;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_20');

         INSERT INTO tt_MOC_DATA_20 ( 
         	SELECT UcifEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_20 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --IF OBJECT_ID('TEMPDB..tt_UpdateTempAssetclassAsse_18') IS NOT NULL  
         -- DROP TABLE  tt_UpdateTempAssetclassAsse_18  
         --select a.CustomerEntityID, count(a.CustomerAcID) CNT
         --into          tt_UpdateTempAssetclassAsse_18
         --from		  pro.accountcal a
         ...
         --INNER JOIN	   pro.ACCOUNTCAL  B
         --ON             A.CustomerEntityID=B.CustomerEntityID
         --INNER JOIN     tt_UpdateTempAssetclassAsse_18 C
         --ON             A.CustomerEntityID=C.CustomerEntityID
         ...
         IF utils.object_id('TEMPDB..tt_UpdateTempAssetclassAsse_18') IS NOT NULL THEN
          --------------------------COBORROWER WORK ADDED BY MANDEEP -------------------------------------------------------------------------
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UpdateTempAssetclassAsse_18 ';
         END IF;
         DELETE FROM tt_UpdateTempAssetclassAsse_18;
         UTILS.IDENTITY_RESET('tt_UpdateTempAssetclassAsse_18');

         INSERT INTO tt_UpdateTempAssetclassAsse_18 ( 
         	SELECT a.CustomerEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_UpdateTempAssetclassAsse_18 C   ON A.CustomerEntityID = C.CustomerEntityID 
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_22112023" TO "ADF_CDR_RBL_STGDB";
