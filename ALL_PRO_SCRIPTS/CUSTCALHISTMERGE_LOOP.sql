...
BEGIN
   ...
   IF tt_TIMEKEY_4  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_TIMEKEY_4;
   UTILS.IDENTITY_RESET('tt_TIMEKEY_4');

   INSERT INTO tt_TIMEKEY_4 SELECT Timekey ,
                                   DATE_ ,
   ...
   LOOP 
      ...
      BEGIN
         ...
         IF tt_custdata  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_custdata;
         UTILS.IDENTITY_RESET('tt_custdata');

         INSERT INTO tt_custdata ( 
         	SELECT * ,
                 'N' IsChanged  ,
         ...
         BEGIN
            ...
            FROM T ,PRO_RBL_MISDB_PROD.CustomerCal_Hist_new O
                   JOIN tt_custdata T   ON O.CustomerEntityId = T.CustomerEntityId
            ...
            FROM TEMP ,tt_custdata TEMP
                   JOIN ( SELECT tt_custdata.CustomerEntityId ,
                                 (v_EntityKeyAc + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                                   FROM DUAL  )  )) EntityKeyNew  
                          FROM tt_custdata 
                           WHERE  tt_custdata.IsChanged IN ( 'C','N' )
