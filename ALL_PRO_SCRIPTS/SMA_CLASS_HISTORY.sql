...
BEGIN
   ...
   ------------------------------------------------------------------------------------------------------
   IF utils.object_id('TEMPDB..tt_TempSMA_Class') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TempSMA_Class ';
   END IF;
   DELETE FROM tt_TempSMA_Class;
   UTILS.IDENTITY_RESET('tt_TempSMA_Class');

   INSERT INTO tt_TempSMA_Class ( 
   ...
   EXECUTE IMMEDIATE ' ALTER TABLE tt_TempSMA_Class 
   ...
   FROM T ,PRO_RBL_MISDB_PROD.SMA_Class_hist O
          JOIN tt_TempSMA_Class T   ON O.AccountEntityID = T.AccountEntityID
   ...
   --SELECT @EntityKeyAc=MAX(EntityKey) FROM  tt_TempSMA_Class
