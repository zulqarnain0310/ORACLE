   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TEMPDB..#AdvCustNPAdetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvCustNPAdetail_9 ';
         END IF;
         DELETE FROM tt_AdvCustNPAdetail_10;
         INSERT INTO tt_AdvCustNPAdetail_9
         ...
         IF tt_NPA_DATA  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_NPA_DATA;
         UTILS.IDENTITY_RESET('tt_NPA_DATA');

         INSERT INTO tt_NPA_DATA ( 
         	SELECT A.* ,
                 UTILS.CONVERT_TO_NUMBER(0,10,0) NewEntityKey  ,
                 'N' IsChanged  
         	  FROM AdvCustNPADetail A
                   JOIN tt_CUSTOMERCAL_11 B   ON A.CustomerEntityId = B.CustomerEntityId
         	 WHERE  A.EffectiveFromTimeKey <= v_TimeKey
                    AND A.EffectiveToTimeKey >= v_TimeKey );
         ----------For New Records
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'N'
         FROM A ,tt_AdvCustNPAdetail_9 A 
         ...
         FROM O ,RBL_MISDB_PROD.AdvCustNPADetail O
                JOIN tt_AdvCustNPAdetail_9 T   ON O.CustomerEntityId = T.CustomerEntityId
         ...
         FROM TEMP ,tt_AdvCustNPAdetail_9 TEMP
                JOIN ( SELECT tt_AdvCustNPAdetail_9.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvCustNPAdetail_9 
                        WHERE  tt_AdvCustNPAdetail_9.IsChanged IN ( 'N','C' )

                                 AND tt_AdvCustNPAdetail_9.EntityKey = 0
                                 OR tt_AdvCustNPAdetail_9.EntityKey IS NULL ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         FROM TEMP ,tt_NPA_DATA TEMP
                JOIN ( SELECT tt_NPA_DATA.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) NewEntityKey  
                       FROM tt_NPA_DATA 
                        WHERE  tt_NPA_DATA.EntityKey = 0
                                 OR tt_NPA_DATA.EntityKey IS NULL
                                 AND tt_NPA_DATA.EffectiveToTimeKey > v_TimeKey ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         IF utils.object_id('TEMPDB..tt_AdvAcFinancialDetail_5') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcFinancialDetail_5 ';
         END IF;
         DELETE FROM tt_AdvAcFinancialDetail_5;
         INSERT INTO tt_AdvAcFinancialDetail_5
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcFinancialDetail O
                JOIN tt_AdvAcFinancialDetail_5 T   ON O.AccountEntityID = T.AccountEntityID
                AND O.EffectiveFromTimeKey <= v_TimeKey
                AND O.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         ------Select * 
         --from tt_AdvAcFinancialDetail_5 A
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcFinancialDetail_5 TEMP
                JOIN ( SELECT tt_AdvAcFinancialDetail_5.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcFinancialDetail_5 
                        WHERE  NVL(tt_AdvAcFinancialDetail_5.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         ---Where Temp.IsChanged in ('N','C')
         ----UPDATE A
         ...
         ----		SELECT COUNT(1), AccountEntityId FROM tt_AdvAcFinancialDetail_5 
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcFinancialDetail
           ...
           ( SELECT ENTITYKEY ,
                    ...
                    AccountBlkCode2 
             FROM tt_AdvAcFinancialDetail_5 T
         ...
         IF utils.object_id('TEMPDB..#AdvAcBalanceDetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcBalanceDetail_9 ';
         END IF;
         DELETE FROM tt_AdvAcBalanceDetail_10;
         INSERT INTO tt_AdvAcBalanceDetail_9
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcBalanceDetail O
                JOIN tt_AdvAcBalanceDetail_9 T   ON O.AccountEntityID = T.AccountEntityID
                AND o.EffectiveFromTimeKey <= v_TimeKey
                AND o.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcBalanceDetail_9 TEMP
                JOIN ( SELECT tt_AdvAcBalanceDetail_9.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcBalanceDetail_9 
                        WHERE  NVL(tt_AdvAcBalanceDetail_9.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         --Where Temp.IsChanged in ('N','C')
         /***************************************************************************************************************/
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ...
           ( SELECT EntityKey ,
                    ...
                    SourceAssetClass ,
                    SourceNpaDate 
             FROM tt_AdvAcBalanceDetail_9 T
              WHERE  NVL(T.IsChanged, 'U') NOT IN ( 'D','K' )
            );
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ( EntityKey, AccountEntityId, AssetClassAlt_Key, BalanceInCurrency, Balance, SignBalance, LastCrDt, OverDue, TotalProv, RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, OverDueSinceDt, MocStatus, MocDate, MocTypeAlt_Key, Old_OverDueSinceDt, Old_OverDue, ORG_TotalProv, IntReverseAmt, UnAppliedIntAmount, PS_Balance, NPS_Balance, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, CreatedBy, UpgradeDate, OverduePrincipal, NotionalInttAmt, PrincipalBalance, Overdueinterest, AdvanceRecovery, PS_NPS_FLAG, DFVAmt, InterestReceivable, OverduePrincipalDt, OverdueIntDt, OverOtherdue, OverdueOtherDt, SourceAssetClass, SourceNpaDate )
