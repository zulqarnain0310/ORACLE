--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_15032023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" /*=========================================  
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_16') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_16 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_16;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_16');

            INSERT INTO tt_TEMPTABLEPANCARD_16 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_16 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_16 A
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_16 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_16 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_16') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_16 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_16;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_16');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_16 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_16 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_16 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
               ...
               FROM DUAL  );
         MERGE INTO ACL 
         USING (SELECT ACL.ROWID row_id, 'ALWYS_NPA', 2, v_ProcessingDate, 'NPA DUE TO CREDIT CARD SETTLEMENT' AS pos_5
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL 
         ...
         MERGE INTO ACL 
         USING (SELECT ACL.ROWID row_id, 'ALWYS_NPA', 'NPA DUE TO CREDIT CARD SETTLEMENT' AS pos_3
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL 
          WHERE AccountBlkCode2 = 'K'
           AND FinalAssetClassAlt_Key > 1) src
         ON ( ACL.ROWID = src.row_id )
         ...
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'ALWYS_NPA', 'NPA DUE TO CREDIT CARD SETTLEMENT' AS pos_3, 2, v_ProcessingDate
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
         ...
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'ALWYS_NPA', 'NPA DUE TO CREDIT CARD SETTLEMENT' AS pos_3
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL AC   ON AC.CustomerEntityID = A.CustomerEntityID 
          WHERE AccountBlkCode2 = 'K'
           AND SysAssetClassAlt_Key > 1) src
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL AC   ON AC.CustomerEntityID = A.CustomerEntityID
                JOIN ExceptionFinalStatusType b   ON AC.CustomerAcID = B.ACID
                AND b.EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                AND b.EffectiveToTimeKey >= v_TIMEKEY 
          WHERE B.StatusType IN ( 'TWO','WO' )
         ...
         --IF OBJECT_ID('TEMPDB..tt_PRVQTRRV_16') IS NOT NULL  
         --DROP TABLE tt_PRVQTRRV_16  
         ...
         --FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_16 B  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID   
         IF utils.object_id('TEMPDB..tt_CustPrevAtrAcl_3') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CustPrevAtrAcl_3 ';
         END IF;
         DELETE FROM tt_CustPrevAtrAcl_3;
         UTILS.IDENTITY_RESET('tt_CustPrevAtrAcl_3');

         INSERT INTO tt_CustPrevAtrAcl_3 ( 
         ...
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_16') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_16 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_16;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_16');

         INSERT INTO tt_PRVQTRRV_16 ( 
         	SELECT A.CustomerEntityId ,
                 SUM(CASE 
                          WHEN c.SysAssetClassAlt_Key > 1 THEN b.CurrentValue
                     ELSE SecurityValueMain
                        END)  SecurityValueMain  
         	  FROM AdvSecurityDetail A
                   ...
                   AND B.EffectiveFromTimeKey <= v_LastQtrDateKey
                   AND B.EffectiveToTimeKey >= v_LastQtrDateKey
                   JOIN tt_CustPrevAtrAcl_3 C   ON C.CustomerEntityID = A.CustomerEntityId
         	  GROUP BY A.CustomerEntityId );
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, SecurityValueMain
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_16 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --UPDATE A SET A.PRVQTRRV= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)  FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_16 B  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_16') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_16 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_16;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_16');

         INSERT INTO tt_CurntQtrRv_16 ( 
         	SELECT CustomerEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_16 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = CurrentValue;
         /*  
         --IF OBJECT_ID('TEMPDB..tt_CurntQtrRv_16') IS NOT NULL  
         --DROP TABLE tt_CurntQtrRv_16  

         --SELECT *    
         --INTO tt_CurntQtrRv_16   
         --FROM dbo.AdvCustSecurityFun(@TIMEKEY)  

         ------UPDATE A SET A.CurntQtrRv= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)   
         ------FROM PRO.CUSTOMERCAL A INNER  JOIN tt_CurntQtrRv_16 B  
         ------ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         */
         ...
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = NULL,
                                      ...
                                      OverdueAmt = 0,
                                      OtherOverdue = 0;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_16') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_16 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_16;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_16');

         INSERT INTO tt_BILL_OVERDUE_16 ( 
         ...
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_16_SCF') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_13 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_13;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_13');

         INSERT INTO tt_BILL_OVERDUE_SCF_13 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
         	  ...
         	  GROUP BY AccountEntityId );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_16_SCF_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_FINAL_13 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_FINAL_13;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_FINAL_13');

         INSERT INTO tt_BILL_OVERDUE_SCF_FINAL_13 ( 
         	SELECT AccountEntityId ,
                 Balance ,
                 overdueinterest ,
         ...
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_SCF_FINAL_13 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = src.BillDueDt,
                                      ...
                                      PrincOverdue = pos_6,
                                      IntOverdue = pos_7;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_16_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_Min_ReviewDt_13 ';
         END IF;
         DELETE FROM tt_BILL_Min_ReviewDt_13;
         UTILS.IDENTITY_RESET('tt_BILL_Min_ReviewDt_13');

         INSERT INTO tt_BILL_Min_ReviewDt_13 ( 
         	SELECT AccountEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_Min_ReviewDt_13 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.ReviewDueDt = ReviewDuedate;
         --=====================================================SCF BILL END===========================================================================
         --IF OBJECT_ID('TEMPDB..tt_BILL_OVERDUE_16_FINAL') IS NOT NULL  
         --    DROP TABLE tt_BILL_OVERDUE_16_FINAL  
         --SELECT AccountEntityId,SUM(BALANCE) BILOVERDUE,MIN(BILLDUEDT) BILLOVERDUEDT  
         -- INTO tt_BILL_OVERDUE_16_FINAL  
         --FROM tt_BILL_OVERDUE_16 GROUP BY AccountEntityId  
         --UPDATE PRO.ACCOUNTCAL SET OVERDUESINCEDT=B.BILLOVERDUEDT ,OverdueAmt  =B.BILOVERDUE  
         --FROM  PRO.ACCOUNTCAL A   
         --INNER JOIN tt_BILL_OVERDUE_16_FINAL B  
         -- ON A.AccountEntityID =B.AccountEntityId  
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_16') IS NOT NULL THEN
          --update a  
         ...
         /* START OF PUI WORK*/
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_16 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_16;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_16');

         INSERT INTO tt_PC_OVERDUE_16 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ----- )<=@PROCESSINGDATE  --  as discussed with Triloki Sir for SMA KIssue - Consider 1 dpd on due date  
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_16_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_16 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_16;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_16');

         INSERT INTO tt_PC_OVERDUE_FINAL_16 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_16 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_16 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         ------------END  
         -----------------------------------------------------------------------------------------------------------------  
         --DROP TABLE IF EXISTS tt_CCOD_90DAYS_INTT_CR_AMT_13  
         --SELECT A.CustomerAcID,  
         ...
         -- INTO tt_CCOD_90DAYS_INTT_CR_AMT_13  
         ...
         IF tt_CCOD_90DAYS_INTT_CR_AMT_13  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CCOD_90DAYS_INTT_CR_AMT_13;
         UTILS.IDENTITY_RESET('tt_CCOD_90DAYS_INTT_CR_AMT_13');

         INSERT INTO tt_CCOD_90DAYS_INTT_CR_AMT_13 ( 
         	SELECT A.CustomerAcID ,
                 SUM(CASE 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL a
                LEFT JOIN tt_CCOD_90DAYS_INTT_CR_AMT_13 b   ON A.CustomerAcID = b.CustomerAcID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_16') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_16 ';
         END IF;
         DELETE FROM tt_Stock_16;
         UTILS.IDENTITY_RESET('tt_Stock_16');

         INSERT INTO tt_Stock_16 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_16_IX ON tt_Stock_16(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_16NON ON tt_Stock_16(AccountEntityId,StkSmtDt)  
         ----IF OBJECT_ID('Tempdb..tt_Stock_162') IS NOT NULL  
         ----DROP TABLE tt_Stock_162  
         ...
         ----   GROUP BY AccountEntityId  
         ----CREATE CLUSTERED INDEX tt_Stock_162_IX ON tt_Stock_162(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_162NON ON tt_Stock_162(AccountEntityId,StkSmtDt)  
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A   
         --INNER  JOIN tt_Stock_16 SD ON A.AccountEntityId=SD.AccountEntityId  
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_31') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_31 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_31;
         INSERT INTO tt_TEMPDerecognisedInterest_31
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_31 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_32') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_32 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_32;
         INSERT INTO tt_TEMPDerecognisedInterest_32
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_32 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_MOC_DATA_15  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_15;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_15');

         INSERT INTO tt_MOC_DATA_15 ( 
         	SELECT UcifEntityID ,
         ...
         , CASE 
         WHEN b.SysNPA_Dt < a.SysNPA_Dt THEN b.SysNPA_Dt
         ELSE NVL(A.SysNPA_Dt, B.SysNPA_Dt)
            END AS pos_4
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_15 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.Asset_Norm = pos_2,
                                      A.MOCTYPE = src.MOCTYPE,
                                      A.SysNPA_Dt ---email Dated 16-06-2022--For some cases there is change in NPA date from 31 March 2022 QTR v/s 31 May 2022 output
                                       = pos_4;
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, CASE 
         WHEN B.SysAssetClassAlt_Key = 1 THEN 'ALWYS_STD'
         ...
         , CASE 
         WHEN b.SysNPA_Dt < a.FinalNpaDt THEN b.SysNPA_Dt
         ELSE NVL(a.FinalNpaDt, b.SysNPA_Dt)
            END AS pos_4
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_MOC_DATA_15 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.Asset_Norm = pos_2,
                                      MOCTYPE = src.MOCTYPE,
                                      FinalNpaDt ---email Dated 16-06-2022--For some cases there is change in NPA date from 31 March 2022 QTR v/s 31 May 2022 output
                                       = pos_4;
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, CASE 
         WHEN B.SysAssetClassAlt_Key = 1 THEN 'ALWYS_STD'
         ...
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'NORMAL', A.InitialAssetClassAlt_Key, A.InitialNpaDt
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN PRO_RBL_MISDB_PROD.BuyoutUploadDetailsCal B   ON A.AccountEntityID = B.AccountEntityId 
          WHERE NVL(A.Asset_Norm, ' ') <> 'NORMAL') src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.Asset_Norm = 'NORMAL',
                                      A.FinalAssetClassAlt_Key = src.InitialAssetClassAlt_Key,
                                      A.FinalNpaDt = src.InitialNpaDt;
         MERGE INTO C 
         USING (SELECT C.ROWID row_id, 'NORMAL', c.SrcAssetClassAlt_Key, c.SrcNPA_Dt
         FROM C ,PRO_RBL_MISDB_PROD.CUSTOMERCAL c
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL A   ON a.CustomerEntityID = c.CustomerEntityID
                JOIN PRO_RBL_MISDB_PROD.BuyoutUploadDetailsCal B   ON A.AccountEntityID = B.AccountEntityId 
          WHERE NVL(C.Asset_Norm, ' ') <> 'NORMAL') src
         ON ( C.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET c.Asset_Norm = 'NORMAL',
                                      c.SysAssetClassAlt_Key = src.SrcAssetClassAlt_Key,
                                      c.SysNPA_Dt = src.SrcNPA_Dt;
         ...
         --IF OBJECT_ID('TEMPDB..tt_UpdateTempAssetclassAsse_13') IS NOT NULL  
         -- DROP TABLE  tt_UpdateTempAssetclassAsse_13  
         --select a.CustomerEntityID, count(a.CustomerAcID) CNT
         --into          tt_UpdateTempAssetclassAsse_13
         --from		  pro.accountcal a
         ...
         --INNER JOIN	   pro.ACCOUNTCAL  B
         --ON             A.CustomerEntityID=B.CustomerEntityID
         --INNER JOIN     tt_UpdateTempAssetclassAsse_13 C
         --ON             A.CustomerEntityID=C.CustomerEntityID
         ...
         IF utils.object_id('TEMPDB..tt_UpdateTempAssetclassAsse_13') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UpdateTempAssetclassAsse_13 ';
         END IF;
         DELETE FROM tt_UpdateTempAssetclassAsse_13;
         UTILS.IDENTITY_RESET('tt_UpdateTempAssetclassAsse_13');

         INSERT INTO tt_UpdateTempAssetclassAsse_13 ( 
         	SELECT a.CustomerEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_UpdateTempAssetclassAsse_13 C   ON A.CustomerEntityID = C.CustomerEntityID 
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15032023" TO "ADF_CDR_RBL_STGDB";
