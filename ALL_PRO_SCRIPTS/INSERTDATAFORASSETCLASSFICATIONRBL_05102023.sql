--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_05102023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" /*=========================================  
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_10') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_10 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_10;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_10');

            INSERT INTO tt_TEMPTABLEPANCARD_10 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_10 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_10 A
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_10 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_10 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_10') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_10 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_10;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_10');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_10 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_10 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_10 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL 
          WHERE AccountBlkCode2 IN ( 'K','E','W' )
           ...
           AND ACL.AccountEntityID NOT IN ( SELECT b.AccountEntityID 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
          ...
          WHERE AccountBlkCode2 IN ( 'K','E','W' )
           ...
           AND AC.AccountEntityID NOT IN ( SELECT b.AccountEntityID 
                                           FROM ExceptionFinalStatusType A
                                                  JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL B   ON A.AccountEntityID = B.AccountEntityID
                                            WHERE  A.EffectiveFromTimeKey <= v_TIMEKEY
                                                     AND A.EffectiveToTimeKey >= v_TIMEKEY
         ...
         IF tt_excepdata_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_excepdata_5;
         UTILS.IDENTITY_RESET('tt_excepdata_5');

         INSERT INTO tt_excepdata_5 ( 
         	SELECT acid ,
         	 ...
         	 WHERE  StatusType IN ( 'TWO','WO','Charge Off','Settlement' )
         	  ...
         	  GROUP BY ACID );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPACID_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPACID_5;
         UTILS.IDENTITY_RESET('tt_EXPACID_5');

         INSERT INTO tt_EXPACID_5 ( 
         	SELECT a.acid ,
                 StatusType ,
                 UTILS.CONVERT_TO_NUMBER(0,19,0) CUSTOMERENTITYID  ,
                 UTILS.CONVERT_TO_NUMBER(0,19,0) UCIFENTITYID  
         	  FROM ExceptionFinalStatusType a
                   JOIN tt_excepdata_5 b   ON a.acid = b.acid
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_ADVACID_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_ADVACID_5;
         UTILS.IDENTITY_RESET('tt_ADVACID_5');

         INSERT INTO tt_ADVACID_5 ( 
         	SELECT AccountEntityId ,
                 MAX(EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM AdvAcBasicDetail A
                   JOIN tt_EXPACID_5 B   ON A.CustomerACID = B.ACID
         	  GROUP BY AccountEntityId );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPCUST_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPCUST_5;
         UTILS.IDENTITY_RESET('tt_EXPCUST_5');

         INSERT INTO tt_EXPCUST_5 ( 
         	SELECT CustomerEntityId ,
                 CustomerACID 
         	  FROM AdvAcBasicDetail A
                   JOIN tt_ADVACID_5 B   ON A.AccountEntityId = B.AccountEntityId
                   AND a.EffectiveFromTimeKey = B.EffectiveFromTimeKey );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPCUST_5ID  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPCUSTID_5;
         UTILS.IDENTITY_RESET('tt_EXPCUSTID_5');

         INSERT INTO tt_EXPCUSTID_5 ( 
         	SELECT A.CustomerEntityId ,
                 MAX(EffectiveFromTimeKey)  EffectiveFromTimeKey  
         	  FROM CustomerBasicDetail A
                   JOIN tt_EXPCUST_5 B   ON A.CustomerEntityId = B.CustomerEntityId
         	  GROUP BY A.CustomerEntityId );
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_EXPUCIC_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_EXPUCIC_5;
         UTILS.IDENTITY_RESET('tt_EXPUCIC_5');

         INSERT INTO tt_EXPUCIC_5 ( 
         	SELECT A.CustomerEntityId ,
                 UcifEntityID 
         	  FROM CustomerBasicDetail A
                   JOIN tt_EXPCUSTID_5 B   ON A.CustomerEntityId = B.CustomerEntityId
                   AND a.EffectiveFromTimeKey = B.EffectiveFromTimeKey );
         ---------------
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, B.CustomerEntityId
         FROM A ,tt_EXPACID_5 A
                JOIN tt_EXPCUST_5 B   ON A.ACID = B.CustomerACID ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,tt_EXPACID_5 A
                JOIN tt_EXPUCIC_5 B   ON A.CUSTOMERENTITYID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_LINKACEXP_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_LINKACEXP_5;
         UTILS.IDENTITY_RESET('tt_LINKACEXP_5');

         INSERT INTO tt_LINKACEXP_5 ( 
         ...
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL
                JOIN tt_EXPACID_5 A   ON ACL.UcifEntityID = A.UcifEntityID
                JOIN tt_LINKACEXP_5 b   ON a.UCIFENTITYID = acl.UcifEntityID 
         ...
         FROM ACL ,PRO_RBL_MISDB_PROD.ACCOUNTCAL ACL
                JOIN tt_EXPACID_5 A   ON ACL.UcifEntityID = A.UcifEntityID
                JOIN tt_LINKACEXP_5 b   ON a.UCIFENTITYID = B.UcifEntityID 
         ...
         --IF OBJECT_ID('TEMPDB..tt_PRVQTRRV_10') IS NOT NULL  
         --DROP TABLE tt_PRVQTRRV_10  
         ...
         --FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_10 B  
         ...
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_10') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_10 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_10;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_10');

         INSERT INTO tt_PRVQTRRV_10 ( 
         	SELECT A.CustomerEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_10 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --UPDATE A SET A.PRVQTRRV= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)  FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_10 B                  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_10') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_10 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_10;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_10');

         INSERT INTO tt_CurntQtrRv_10 ( 
         	SELECT CustomerEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_10 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key <> 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = CurrentValue;
         ------- ----Added By Mandeep (Security Perfection)
         DELETE FROM tt_CurntQtrRv_Finacle_8;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_Finacle_8');

         INSERT INTO tt_CurntQtrRv_Finacle_8 ( 
         	SELECT CustomerEntityId ,
                 SUM(b.CurrentValue)  CurrentValue  
         	  FROM AdvSecurityDetail A
                   JOIN AdvSecurityValueDetail B   ON A.SecurityEntityID = B.SecurityEntityID
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_Finacle_8 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key = 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = src.CurrentValue;
         /*                  
         --IF OBJECT_ID('TEMPDB..tt_CurntQtrRv_10') IS NOT NULL                  
         --DROP TABLE tt_CurntQtrRv_10                  

         --SELECT *                    
         --INTO tt_CurntQtrRv_10                   
         --FROM dbo.AdvCustSecurityFun(@TIMEKEY)                  

         ------UPDATE A SET A.CurntQtrRv= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)                   
         ------FROM PRO.CUSTOMERCAL A INNER  JOIN tt_CurntQtrRv_10 B                  
         ------ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         */
         ...
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = NULL,
                                      ...
                                      OverdueAmt = 0,
                                      OtherOverdue = 0;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_10') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_10 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_10;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_10');

         INSERT INTO tt_BILL_OVERDUE_10 ( 
         	SELECT AccountEntityID ,
                 BILLENTITYID ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_10_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_FINAL_10 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_FINAL_10;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_FINAL_10');

         INSERT INTO tt_BILL_OVERDUE_FINAL_10 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  BILOVERDUE  ,
                 MIN(BILLDUEDT)  BILLOVERDUEDT  
         	  FROM tt_BILL_OVERDUE_10 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.BILLOVERDUEDT, B.BILOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_FINAL_10 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET OVERDUESINCEDT = src.BILLOVERDUEDT,
                                      OverdueAmt = src.BILOVERDUE;
         --=====SCF SOURCE SYSTEM CHANGES==================================================================================================
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_10_SCF') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_8 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_8;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_8');

         INSERT INTO tt_BILL_OVERDUE_SCF_8 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
         	  ...
         	  GROUP BY AccountEntityId );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_10_SCF_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_FINAL_8 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_FINAL_8;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_FINAL_8');

         INSERT INTO tt_BILL_OVERDUE_SCF_FINAL_8 ( 
         	SELECT AccountEntityId ,
                 Balance ,
                 overdueinterest ,
         ...
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_SCF_FINAL_8 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = src.BillDueDt,
                                      ...
                                      PrincOverdue = pos_6,
                                      IntOverdue = pos_7;
         IF utils.object_id('TEMPDB..tt_BILL_Min_ReviewDt_8') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_Min_ReviewDt_8 ';
         END IF;
         DELETE FROM tt_BILL_Min_ReviewDt_8;
         UTILS.IDENTITY_RESET('tt_BILL_Min_ReviewDt_8');

         INSERT INTO tt_BILL_Min_ReviewDt_8 ( 
         	SELECT AccountEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_Min_ReviewDt_8 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.ReviewDueDt = ReviewDuedate;
         --=====================================================SCF BILL END===========================================================================
         --IF OBJECT_ID('TEMPDB..tt_BILL_OVERDUE_10_FINAL') IS NOT NULL  
         --    DROP TABLE tt_BILL_OVERDUE_10_FINAL  
         --SELECT AccountEntityId,SUM(BALANCE) BILOVERDUE,MIN(BILLDUEDT) BILLOVERDUEDT  
         -- INTO tt_BILL_OVERDUE_10_FINAL  
         --FROM tt_BILL_OVERDUE_10 GROUP BY AccountEntityId  
         --UPDATE PRO.ACCOUNTCAL SET OVERDUESINCEDT=B.BILLOVERDUEDT ,OverdueAmt  =B.BILOVERDUE  
         --FROM  PRO.ACCOUNTCAL A   
         --INNER JOIN tt_BILL_OVERDUE_10_FINAL B  
         -- ON A.AccountEntityID =B.AccountEntityId  
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_10') IS NOT NULL THEN
          --update a  
         ...
         /* START OF PUI WORK*/
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_10 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_10;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_10');

         INSERT INTO tt_PC_OVERDUE_10 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ----- )<=@PROCESSINGDATE  --  as discussed with Triloki Sir for SMA KIssue - Consider 1 dpd on due date  
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_10_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_10 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_10;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_10');

         INSERT INTO tt_PC_OVERDUE_FINAL_10 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_10 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_10 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         ------------END  
         -----------------------------------------------------------------------------------------------------------------  
         --DROP TABLE IF EXISTS tt_CCOD_90DAYS_INTT_CR_AMT_8  
         --SELECT A.CustomerAcID,  
         ...
         -- INTO tt_CCOD_90DAYS_INTT_CR_AMT_8  
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_CCOD_90DAYS_INTT_CR_AMT_8  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CCOD_90DAYS_INTT_CR_AMT_8;
         UTILS.IDENTITY_RESET('tt_CCOD_90DAYS_INTT_CR_AMT_8');

         INSERT INTO tt_CCOD_90DAYS_INTT_CR_AMT_8 ( 
         	SELECT A.CustomerAcID ,
                 SUM(CASE 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL a
                LEFT JOIN tt_CCOD_90DAYS_INTT_CR_AMT_8 b   ON A.CustomerAcID = b.CustomerAcID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_10') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_10 ';
         END IF;
         DELETE FROM tt_Stock_10;
         UTILS.IDENTITY_RESET('tt_Stock_10');

         INSERT INTO tt_Stock_10 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_10_IX ON tt_Stock_10(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_10NON ON tt_Stock_10(AccountEntityId,StkSmtDt)  
         ----IF OBJECT_ID('Tempdb..tt_Stock_102') IS NOT NULL  
         ----DROP TABLE tt_Stock_102  
         ...
         ----   GROUP BY AccountEntityId  
         ----CREATE CLUSTERED INDEX tt_Stock_102_IX ON tt_Stock_102(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_102NON ON tt_Stock_102(AccountEntityId,StkSmtDt)  
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A   
         --INNER  JOIN tt_Stock_10 SD ON A.AccountEntityId=SD.AccountEntityId  
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_19') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_19 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_19;
         INSERT INTO tt_TEMPDerecognisedInterest_19
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_19 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_20') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_20 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_20;
         INSERT INTO tt_TEMPDerecognisedInterest_20
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_20 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_ADHOC_UcifEntityID_STD_5  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_ADHOC_UcifEntityID_STD_5;
         UTILS.IDENTITY_RESET('tt_ADHOC_UcifEntityID_STD_5');

         INSERT INTO tt_ADHOC_UcifEntityID_STD_5 ( 
         	SELECT UcifEntityID 
         ...
         WITH CTE_A AS ( SELECT CUSTOMERENTITYID 
           FROM tt_ADHOC_UcifEntityID_STD_5 A
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_MOC_DATA_10  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_10;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_10');

         INSERT INTO tt_MOC_DATA_10 ( 
         	SELECT UcifEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_10 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --IF OBJECT_ID('TEMPDB..tt_UpdateTempAssetclassAsse_8') IS NOT NULL  
         -- DROP TABLE  tt_UpdateTempAssetclassAsse_8  
         --select a.CustomerEntityID, count(a.CustomerAcID) CNT
         --into          tt_UpdateTempAssetclassAsse_8
         --from		  pro.accountcal a
         ...
         --INNER JOIN	   pro.ACCOUNTCAL  B
         --ON             A.CustomerEntityID=B.CustomerEntityID
         --INNER JOIN     tt_UpdateTempAssetclassAsse_8 C
         --ON             A.CustomerEntityID=C.CustomerEntityID
         ...
         IF utils.object_id('TEMPDB..tt_UpdateTempAssetclassAsse_8') IS NOT NULL THEN
          --------------------------COBORROWER WORK ADDED BY MANDEEP -------------------------------------------------------------------------
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UpdateTempAssetclassAsse_8 ';
         END IF;
         DELETE FROM tt_UpdateTempAssetclassAsse_8;
         UTILS.IDENTITY_RESET('tt_UpdateTempAssetclassAsse_8');

         INSERT INTO tt_UpdateTempAssetclassAsse_8 ( 
         	SELECT a.CustomerEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_UpdateTempAssetclassAsse_8 C   ON A.CustomerEntityID = C.CustomerEntityID 
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_05102023" TO "ADF_CDR_RBL_STGDB";
