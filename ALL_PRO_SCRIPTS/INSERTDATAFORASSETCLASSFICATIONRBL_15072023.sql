--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_15072023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" /*=========================================  
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_17') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_17 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_17;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_17');

            INSERT INTO tt_TEMPTABLEPANCARD_17 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_17 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_17 A
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_17 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_17 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_17') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_17 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_17;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_17');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_17 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_17 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_17 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         --IF OBJECT_ID('TEMPDB..tt_PRVQTRRV_17') IS NOT NULL  
         --DROP TABLE tt_PRVQTRRV_17  
         ...
         --FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_17 B  
         ...
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_17') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_17 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_17;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_17');

         INSERT INTO tt_PRVQTRRV_17 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_17 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --UPDATE A SET A.PRVQTRRV= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)  FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_17 B                  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_17') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_17 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_17;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_17');

         INSERT INTO tt_CurntQtrRv_17 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_17 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key <> 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = CurrentValue;
         ------- ----Added By Mandeep (Security Perfection)
         DELETE FROM tt_CurntQtrRv_Finacle_12;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_Finacle_12');

         INSERT INTO tt_CurntQtrRv_Finacle_12 ( 
         	SELECT CustomerEntityId ,
                 SUM(b.CurrentValue)  CurrentValue  
         	  FROM AdvSecurityDetail A
                   JOIN AdvSecurityValueDetail B   ON A.SecurityEntityID = B.SecurityEntityID
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_Finacle_12 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key = 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = src.CurrentValue;
         /*                  
         --IF OBJECT_ID('TEMPDB..tt_CurntQtrRv_17') IS NOT NULL                  
         --DROP TABLE tt_CurntQtrRv_17                  

         --SELECT *                    
         --INTO tt_CurntQtrRv_17                   
         --FROM dbo.AdvCustSecurityFun(@TIMEKEY)                  

         ------UPDATE A SET A.CurntQtrRv= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)                   
         ------FROM PRO.CUSTOMERCAL A INNER  JOIN tt_CurntQtrRv_17 B                  
         ------ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         */
         ...
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = NULL,
                                      ...
                                      OverdueAmt = 0,
                                      OtherOverdue = 0;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_17') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_17 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_17;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_17');

         INSERT INTO tt_BILL_OVERDUE_17 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_17_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_FINAL_16 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_FINAL_16;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_FINAL_16');

         INSERT INTO tt_BILL_OVERDUE_FINAL_16 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  BILOVERDUE  ,
                 MIN(BILLDUEDT)  BILLOVERDUEDT  
         	  FROM tt_BILL_OVERDUE_17 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.BILLOVERDUEDT, B.BILOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_FINAL_16 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET OVERDUESINCEDT = src.BILLOVERDUEDT,
                                      OverdueAmt = src.BILOVERDUE;
         --=====SCF SOURCE SYSTEM CHANGES==================================================================================================
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_17_SCF') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_14 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_14;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_14');

         INSERT INTO tt_BILL_OVERDUE_SCF_14 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
         	  ...
         	  GROUP BY AccountEntityId );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_17_SCF_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_FINAL_14 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_FINAL_14;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_FINAL_14');

         INSERT INTO tt_BILL_OVERDUE_SCF_FINAL_14 ( 
         	SELECT AccountEntityId ,
                 Balance ,
                 overdueinterest ,
         ...
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_SCF_FINAL_14 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = src.BillDueDt,
                                      ...
                                      PrincOverdue = pos_6,
                                      IntOverdue = pos_7;
         IF utils.object_id('TEMPDB..tt_BILL_Min_ReviewDt_14') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_Min_ReviewDt_14 ';
         END IF;
         DELETE FROM tt_BILL_Min_ReviewDt_14;
         UTILS.IDENTITY_RESET('tt_BILL_Min_ReviewDt_14');

         INSERT INTO tt_BILL_Min_ReviewDt_14 ( 
         	SELECT AccountEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_Min_ReviewDt_14 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.ReviewDueDt = ReviewDuedate;
         --=====================================================SCF BILL END===========================================================================
         --IF OBJECT_ID('TEMPDB..tt_BILL_OVERDUE_17_FINAL') IS NOT NULL  
         --    DROP TABLE tt_BILL_OVERDUE_17_FINAL  
         --SELECT AccountEntityId,SUM(BALANCE) BILOVERDUE,MIN(BILLDUEDT) BILLOVERDUEDT  
         -- INTO tt_BILL_OVERDUE_17_FINAL  
         --FROM tt_BILL_OVERDUE_17 GROUP BY AccountEntityId  
         --UPDATE PRO.ACCOUNTCAL SET OVERDUESINCEDT=B.BILLOVERDUEDT ,OverdueAmt  =B.BILOVERDUE  
         --FROM  PRO.ACCOUNTCAL A   
         --INNER JOIN tt_BILL_OVERDUE_17_FINAL B  
         -- ON A.AccountEntityID =B.AccountEntityId  
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_17') IS NOT NULL THEN
          --update a  
         ...
         /* START OF PUI WORK*/
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_17 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_17;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_17');

         INSERT INTO tt_PC_OVERDUE_17 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ----- )<=@PROCESSINGDATE  --  as discussed with Triloki Sir for SMA KIssue - Consider 1 dpd on due date  
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_17_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_17 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_17;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_17');

         INSERT INTO tt_PC_OVERDUE_FINAL_17 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_17 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_17 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         ------------END  
         -----------------------------------------------------------------------------------------------------------------  
         --DROP TABLE IF EXISTS tt_CCOD_90DAYS_INTT_CR_AMT_14  
         --SELECT A.CustomerAcID,  
         ...
         -- INTO tt_CCOD_90DAYS_INTT_CR_AMT_14  
         ...
         IF tt_CCOD_90DAYS_INTT_CR_AMT_14  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CCOD_90DAYS_INTT_CR_AMT_14;
         UTILS.IDENTITY_RESET('tt_CCOD_90DAYS_INTT_CR_AMT_14');

         INSERT INTO tt_CCOD_90DAYS_INTT_CR_AMT_14 ( 
         	SELECT A.CustomerAcID ,
                 SUM(CASE 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL a
                LEFT JOIN tt_CCOD_90DAYS_INTT_CR_AMT_14 b   ON A.CustomerAcID = b.CustomerAcID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_17') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_17 ';
         END IF;
         DELETE FROM tt_Stock_17;
         UTILS.IDENTITY_RESET('tt_Stock_17');

         INSERT INTO tt_Stock_17 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_17_IX ON tt_Stock_17(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_17NON ON tt_Stock_17(AccountEntityId,StkSmtDt)  
         ----IF OBJECT_ID('Tempdb..tt_Stock_172') IS NOT NULL  
         ----DROP TABLE tt_Stock_172  
         ...
         ----   GROUP BY AccountEntityId  
         ----CREATE CLUSTERED INDEX tt_Stock_172_IX ON tt_Stock_172(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_172NON ON tt_Stock_172(AccountEntityId,StkSmtDt)  
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A   
         --INNER  JOIN tt_Stock_17 SD ON A.AccountEntityId=SD.AccountEntityId  
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_33') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_33 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_33;
         INSERT INTO tt_TEMPDerecognisedInterest_33
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_33 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_34') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_34 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_34;
         INSERT INTO tt_TEMPDerecognisedInterest_34
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_34 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_MOC_DATA_16  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_16;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_16');

         INSERT INTO tt_MOC_DATA_16 ( 
         	SELECT UcifEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_16 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --IF OBJECT_ID('TEMPDB..tt_UpdateTempAssetclassAsse_14') IS NOT NULL  
         -- DROP TABLE  tt_UpdateTempAssetclassAsse_14  
         --select a.CustomerEntityID, count(a.CustomerAcID) CNT
         --into          tt_UpdateTempAssetclassAsse_14
         --from		  pro.accountcal a
         ...
         --INNER JOIN	   pro.ACCOUNTCAL  B
         --ON             A.CustomerEntityID=B.CustomerEntityID
         --INNER JOIN     tt_UpdateTempAssetclassAsse_14 C
         --ON             A.CustomerEntityID=C.CustomerEntityID
         ...
         IF utils.object_id('TEMPDB..tt_UpdateTempAssetclassAsse_14') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UpdateTempAssetclassAsse_14 ';
         END IF;
         DELETE FROM tt_UpdateTempAssetclassAsse_14;
         UTILS.IDENTITY_RESET('tt_UpdateTempAssetclassAsse_14');

         INSERT INTO tt_UpdateTempAssetclassAsse_14 ( 
         	SELECT a.CustomerEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_UpdateTempAssetclassAsse_14 C   ON A.CustomerEntityID = C.CustomerEntityID 
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_15072023" TO "ADF_CDR_RBL_STGDB";
