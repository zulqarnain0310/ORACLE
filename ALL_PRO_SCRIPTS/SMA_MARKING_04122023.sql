--------------------------------------------------------
--  DDL for Procedure SMA_MARKING_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."SMA_MARKING_04122023" 
(
  v_TIMEKEY IN NUMBER
)
AS
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TEMPDB..tt_DPD_35') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_DPD_35 ';
         END IF;
         DELETE FROM tt_DPD_35;
         UTILS.IDENTITY_RESET('tt_DPD_35');

         INSERT INTO tt_DPD_35 ( 
         ...
         ----/* CALCULATE MAX DPD */  
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_129') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_129 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_129;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_129');

         INSERT INTO tt_TEMPTABLE_129 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
              ...
              --AND ABD.ReferencePeriod=91   
                ...
                JOIN tt_DPD_35 dpd   ON dpd.AccountEntityId = a.AccountEntityID
         ...
         ------ INNER JOIN tt_DPD_35 dpd on dpd.AccountEntityId=a.AccountEntityId  
         ------   --LEFT JOIN DIMPRODUCT C ON C.PRODUCTALT_KEY=A.PRODUCTALT_KEY     
         ------   --AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'    
         ------   --AND isnull(C.ProductSubGroup,'N') NOT in('KCC')  
         ------   --and c.NPANorms='DPD91'  
         ...
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_129_SMACLASS') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_SMACLASS_3 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_SMACLASS_3;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_SMACLASS_3');

         INSERT INTO tt_TEMPTABLE_SMACLASS_3 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_TEMPTABLE_SMACLASS_3 B   ON A.CustomerEntityID = B.CustomerEntityID 
         ...
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.FLGSMA = 'Y';
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_129_SMACLASSUcif') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_SMACLASSUcif_3 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_SMACLASSUcif_3;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_SMACLASSUcif_3');

         INSERT INTO tt_TEMPTABLE_SMACLASSUcif_3 ( 
         	SELECT A.UCIF_ID ,
                 MAX(CASE 
                          WHEN SMA_CLASS = 'SMA_0' THEN 1
                          WHEN SMA_CLASS = 'SMA_1' THEN 2
         ...
         USING (SELECT A.ROWID row_id, B.MAXSMA_CLASS, B.SMA_Dt
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_TEMPTABLE_SMACLASSUcif_3 B   ON A.UCIF_ID = B.UCIF_ID 
          WHERE A.FlgSMA = 'Y') src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.SMA_CLASS_KEY = src.MAXSMA_CLASS,
                                      A.SMA_DT = src.SMA_Dt;
         ...
         END;
         END IF;
         IF utils.object_id('TEMPDB..tt_SMACLASS_3') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_SMACLASS_3 ';
         END IF;
         DELETE FROM tt_SMACLASS_3;
         UTILS.IDENTITY_RESET('tt_SMACLASS_3');

         INSERT INTO tt_SMACLASS_3 ( 
         	SELECT A.CustomerAcID ,
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.SMA_MOVEMENT_HISTORY
           ...
           ( SELECT v_TIMEKEY ,
             ...
             FROM PRO_RBL_MISDB_PROD.PrevSMAStatus A
                    RIGHT JOIN tt_SMACLASS_3 B   ON A.CustomerAcID = B.CustomerAcID
         ...
         UPDATE PRO_RBL_MISDB_PROD.ACCOUNTCAL
          ...
          WHERE  FinalAssetClassAlt_Key = 6
           AND SMA_CLASS IS NULL;
         --IF OBJECT_ID('TEMPDB..tt_ACCOUNT_MOVEMENT_HISTORY_3') IS NOT NULL  
         --  DROP TABLE tt_ACCOUNT_MOVEMENT_HISTORY_3  
         --  SELECT CustomerAcID,FinalAssetClassAlt_Key,FinalNpaDt,EffectiveFromTimeKey,EffectiveToTimeKeY  INTO tt_ACCOUNT_MOVEMENT_HISTORY_3  
         --  FROM  PRO.AccountCal   
         --  ALTER TABLE tt_ACCOUNT_MOVEMENT_HISTORY_3 ADD MATCH CHAR(1)  
         --  UPDATE tt_ACCOUNT_MOVEMENT_HISTORY_3 SET MATCH='N'  
         -- UPDATE A SET MATCH='Y'  
         --  FROM  tt_ACCOUNT_MOVEMENT_HISTORY_3 A  
         ...
         --  FROM tt_ACCOUNT_MOVEMENT_HISTORY_3 A   INNER JOIN PRO.ACCOUNT_MOVEMENT_HISTORY B  
         --  ON A.CustomerAcID=B.CustomerAcID   
         --  WHERE A.MATCH='D' AND B.EffectiveToTimeKey=49999  
         -- INSERT INTO PRO.ACCOUNT_MOVEMENT_HISTORY (CustomerAcID,FinalAssetClassAlt_Key,FinalNpaDt,EffectiveFromTimeKey,EffectiveToTimeKeY)  
         -- select CustomerAcID,FinalAssetClassAlt_Key,FinalNpaDt,@TIMEKEY,49999  
         -- from tt_ACCOUNT_MOVEMENT_HISTORY_3 where MATCH  in ('N','D')  
         BEGIN
            SELECT 1 INTO v_temp
              FROM DUAL
             WHERE EXISTS ( SELECT 1 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_ACCOUNT_MOVEMENT_HISTORY_3') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ACCOUNT_MOVEMENT_HISTORY_3 ';
            END IF;
            DELETE FROM tt_ACCOUNT_MOVEMENT_HISTORY_3;
            -- ADDED BY AMAR ON 13102021 FOR OPTIMISE - TABIKNG TME TO UPDATE   
            -- ADDED BY AMAR ON 13102021 FOR OPTIMISE - TABIKNG TME TO UPDATE  
            INSERT INTO tt_ACCOUNT_MOVEMENT_HISTORY_3
            ...
            FROM AA ,PRO_RBL_MISDB_PROD.ACCOUNT_MOVEMENT_HISTORY AA
                   LEFT JOIN tt_ACCOUNT_MOVEMENT_HISTORY_3 B   ON AA.CustomerAcID = B.CustomerAcID
                   AND B.EffectiveToTimeKey = 49999 
            ...
            FROM AA ,PRO_RBL_MISDB_PROD.ACCOUNT_MOVEMENT_HISTORY AA 
             WHERE AA.EffectiveToTimeKey = 49999
              ...
              AND EXISTS ( SELECT 1 
                           FROM tt_ACCOUNT_MOVEMENT_HISTORY_3 BB
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_Customer_MOVEMENT_HISTOR_3') IS NOT NULL THEN
             -----------------Added for DashBoard 04-03-2021  
            --Update BANDAUDITSTATUS set CompletedCount=CompletedCount+1 where BandName='ASSET CLASSIFICATION'  
            EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Customer_MOVEMENT_HISTOR_3 ';
            END IF;
            DELETE FROM tt_Customer_MOVEMENT_HISTOR_3;
            -- ADDED BY AMAR ON 13102021 FOR OPTIMISE - TABIKNG TME TO UPDATE   
            -- ADDED BY AMAR ON 13102021 FOR OPTIMISE - TABIKNG TME TO UPDATE  
            INSERT INTO tt_Customer_MOVEMENT_HISTOR_3
            ...
            , MovementToDate -- ADDED BY AMAR ON 13102021 FOR OPTIMISE - TABIKNG TME TO UPDATE  
              ...
              ( SELECT A.UCIF_ID ,
                ...
                FROM tt_Customer_MOVEMENT_HISTOR_3 A
            ...
            FROM AA ,PRO_RBL_MISDB_PROD.CUSTOMER_MOVEMENT_HISTORY AA
                   LEFT JOIN tt_Customer_MOVEMENT_HISTOR_3 B   ON AA.SourceSystemCustomerID = B.SourceSystemCustomerID
                   AND B.EffectiveToTimeKey = 49999 
             WHERE AA.EffectiveToTimeKey = 49999
            ...
            FROM AA ,PRO_RBL_MISDB_PROD.CUSTOMER_MOVEMENT_HISTORY AA 
             WHERE AA.EffectiveToTimeKey = 49999
              AND AA.EffectiveFromTimeKey < v_TIMEKEY
              AND EXISTS ( SELECT 1 
                           FROM tt_Customer_MOVEMENT_HISTOR_3 BB
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."SMA_MARKING_04122023" TO "ADF_CDR_RBL_STGDB";
