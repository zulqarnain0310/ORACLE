...
BEGIN
   ...
   SELECT TimeKey 
     ...
     FROM RBL_MISDB_PROD.Automate_Advances 
    WHERE  EXT_FLG = 'Y';
   IF utils.object_id('TEMPDB..tt_ContinousExcessSecDtAcco') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ContinousExcessSecDtAcco ';
   END IF;
   DELETE FROM tt_ContinousExcessSecDtAcco;
   UTILS.IDENTITY_RESET('tt_ContinousExcessSecDtAcco');

   INSERT INTO tt_ContinousExcessSecDtAcco ( 
   	SELECT CustomerAcID 
   	  FROM tt_AccountCal_10 A
             JOIN ( SELECT C.AccountEntityID ,
                    ...
                    FROM CurDat_RBL_MISDB_PROD.AdvSecurityValueDetail B
                           ...
                           JOIN tt_AccountCal_10 C   ON Advsec.AccountEntityID = C.AccountEntityID
   ...
   INSERT INTO PRO_RBL_MISDB_PROD.ContinousExcessSecDtAccountCal
     ...
     ( SELECT B.CustomerAcID CustomerAcID  ,
       ...
       FROM tt_ContinousExcessSecDtAcco A
              JOIN tt_AccountCal_10 B   ON A.CustomerACID = B.CustomerACID );
   IF utils.object_id('TEMPDB..tt_ContinousExcessSecDtAccoEXP') IS NOT NULL THEN
    EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_ContinousExcessSecDtAcco_2 ';
   END IF;
   DELETE FROM tt_ContinousExcessSecDtAcco_2;
   UTILS.IDENTITY_RESET('tt_ContinousExcessSecDtAcco_2');

   INSERT INTO tt_ContinousExcessSecDtAcco_2 ( 
   	SELECT CustomerAcID 
   	  FROM tt_AccountCal_10 A
             JOIN ( SELECT C.AccountEntityID ,
                           SUM(NVL(CurrentValue, 0))  CurrentValue  
   ...
   FROM A ,PRO_RBL_MISDB_PROD.ContinousExcessSecDtAccountCal A
          LEFT JOIN ( SELECT A.CustomerAcID 
                      FROM PRO_RBL_MISDB_PROD.ContinousExcessSecDtAccountCal a
                             JOIN tt_ContinousExcessSecDtAcco_2 b   ON a.CustomerAcID = b.CustomerAcID ) C   ON A.CustomerAcID = C.CustomerAcID 
