--------------------------------------------------------
--  DDL for Procedure INSERTDATAINTOHIST_TABLE_OPT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" 
(
  v_TIMEKEY IN NUMBER
)
AS
...
BEGIN
   ...
   BEGIN
      ...
      -- LOAN BUYOUT HIST MERGE MERGE --
       			UPDATE T
      				SET T.IsChanged='E'
      			FROM [PRO].[BuyoutUploadDetailsCal_Hist] AS O
      			INNER JOIN PRO.BuyoutUploadDetailsCal AS T
      				ON O.AccountEntityId=T.AccountEntityId
      				AND O.EffectiveToTimeKey=49999
      			...
      			INNER JOIN [PRO].[BuyoutUploadDetailsCal] AS T
      				...
      				AND O.EffectiveToTimeKey=49999
      				WHERE (     
      						 ISNULL(O.DateofData,'1900-01-01')<>ISNULL(T.DateofData,'1900-01-01')
      						OR ISNULL(O.ReportDate,'1900-01-01')<>ISNULL(T.ReportDate,'1900-01-01')
      						OR ISNULL(O.SchemeCode,'')<>ISNULL(T.SchemeCode,'')
      						OR ISNULL(O.NPA_ClassSeller,'')<>ISNULL(T.NPA_ClassSeller,'')
      						OR ISNULL(O.NPA_DateSeller,'1900-01-01')<>ISNULL(T.NPA_DateSeller,'1900-01-01')
      						OR ISNULL(O.DPD_Seller,0)<>ISNULL(T.DPD_Seller,0)
      						OR ISNULL(O.PeakDPD,0)<>ISNULL(T.PeakDPD,0)
      						OR ISNULL(O.PeakDPD_Date,'1900-01-01')<>ISNULL(T.PeakDPD_Date,'1900-01-01')
      						OR ISNULL(O.NPA_DueToMainAdv,'')<>ISNULL(T.NPA_DueToMainAdv,'')
      						OR ISNULL(O.NPA_EffectedToMainAdv,'')<>ISNULL(T.NPA_EffectedToMainAdv,'')
      						OR ISNULL(O.NPA_Flag,'')<>ISNULL(T.NPA_Flag,'')
      					)

      			UPDATE A SET 
      				A.IsChanged='U'
      			FROM PRO.BuyoutUploadDetailsCal A
      			INNER JOIN [PRO].[BuyoutUploadDetailsCal_Hist] B 
      				ON B.AccountEntityId=A.AccountEntityId 
      			WHERE B.EffectiveFromTimeKey= @TimeKey
      				and A.IsChanged='C'
      			...
      			UPDATE O
      				SET  O.DateofData 			 = T.DateofData 
      					,O.ReportDate 			 = T.ReportDate 
      					,O.SchemeCode 			 = T.SchemeCode 
      					,O.NPA_ClassSeller 		 = T.NPA_ClassSeller 
      					,O.NPA_DateSeller 		 = T.NPA_DateSeller 
      					,O.DPD_Seller 			 = T.DPD_Seller 
      					,O.PeakDPD 				 = T.PeakDPD 
      					,O.PeakDPD_Date 		 = T.PeakDPD_Date 
      					,O.NPA_DueToMainAdv 	 = T.NPA_DueToMainAdv 
      					,O.NPA_EffectedToMainAdv = T.NPA_EffectedToMainAdv 
      					,O.NPA_Flag				 = T.NPA_Flag

      			FROM PRO.BuyoutUploadDetailsCal_Hist O
      				INNER JOIN PRO.BuyoutUploadDetailsCal T 
      					ON O.AccountEntityId=T.AccountEntityId 
      				WHERE O.EffectiveFromTimeKey=@TimeKey
      					AND T.IsChanged='U'
      		...
      		SET 
      			EffectiveToTimeKey = @VEFFECTIVETO
      		FROM PRO.BuyoutUploadDetailsCal_Hist AA
      		WHERE AA.EffectiveToTimeKey = 49999
      		AND NOT EXISTS (SELECT 1 FROM PRO.BuyoutUploadDetailsCal BB
      							WHERE AA.AccountEntityId=BB.AccountEntityId 
      						)

      		--*************************************************************************************************************--

      			INSERT INTO PRO.BuyoutUploadDetailsCal_Hist
      				(
      					DateofData
      					,ReportDate
      					,CustomerAcID
      			...
      			SELECT
      					...
      					,NPA_EffectedToMainAdv
      					,NPA_Flag
      				FROM PRO.BuyoutUploadDetailsCal  T Where ISNULL(T.IsChanged,'N') IN('C','N')
      ...
      DELETE PRO.Accountcal_hist_DPD WHERE TimeKey=@TIMEKEY 

      INSERT INTO PRO.Accountcal_hist_DPD
      (
      AccountEntityid
      ,TimeKey
      ,DPD_IntService
      ...
      IF EXISTS (SELECT 1 FROM [PRO].[CoBorrowerCal_Hist] WHERE TIMEKEY=@TIMEKEY )
      BEGIN 
         DELETE FROM [PRO].[CoBorrowerCal_Hist] WHERE TIMEKEY=@TIMEKEY
      END

      INSERT INTO [PRO].[CoBorrowerCal_Hist]
      SELECT * FROM [PRO].[CoBorrowerCal]
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAINTOHIST_TABLE_OPT" TO "ADF_CDR_RBL_STGDB";
