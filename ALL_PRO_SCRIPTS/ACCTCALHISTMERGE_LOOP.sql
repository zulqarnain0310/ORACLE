...
BEGIN
   ...
   IF tt_TIMEKEY  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_TIMEKEY;
   UTILS.IDENTITY_RESET('tt_TIMEKEY');

   INSERT INTO tt_TIMEKEY SELECT Timekey ,
                                 DATE_ ,
   ...
   LOOP 
      ...
      BEGIN
         ...
         IF tt_acdata  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_acdata;
         UTILS.IDENTITY_RESET('tt_acdata');

         INSERT INTO tt_acdata ( 
         	SELECT * ,
                 'N' IsChanged  ,
         ...
         BEGIN
            ...
            FROM T ,PRO_RBL_MISDB_PROD.AccountCal_Hist_new O
                   JOIN tt_acdata T   ON O.AccountEntityID = T.AccountEntityID
            ...
            FROM TEMP ,tt_acdata TEMP
                   JOIN ( SELECT tt_acdata.AccountEntityId ,
                                 (v_EntityKeyAc + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                                   FROM DUAL  )  )) EntityKeyNew  
                          FROM tt_acdata 
                           WHERE  tt_acdata.IsChanged IN ( 'C','N' )
