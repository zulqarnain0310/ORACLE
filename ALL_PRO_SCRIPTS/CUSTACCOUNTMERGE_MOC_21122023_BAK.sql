--------------------------------------------------------
--  DDL for Procedure CUSTACCOUNTMERGE_MOC_21122023_BAK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" 
(
  v_TIMEKEY IN NUMBER
)
AS
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TEMPDB..#AdvCustNPAdetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvCustNPAdetail_15 ';
         END IF;
         DELETE FROM tt_AdvCustNPAdetail_16;
         INSERT INTO tt_AdvCustNPAdetail_15
         ...
         IF tt_NPA_DATA_4  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_NPA_DATA_4;
         UTILS.IDENTITY_RESET('tt_NPA_DATA_4');

         INSERT INTO tt_NPA_DATA_4 ( 
         	SELECT A.* ,
         ...
         FROM A ,tt_AdvCustNPAdetail_15 A 
          WHERE NOT EXISTS ( SELECT 1 
                             FROM tt_NPA_DATA_4 B
         ...
         FROM O ,RBL_MISDB_PROD.AdvCustNPADetail O
                JOIN tt_AdvCustNPAdetail_15 T   ON O.CustomerEntityId = T.CustomerEntityId
         ...
         FROM AA ,RBL_MISDB_PROD.AdvCustNPADetail AA 
          WHERE AA.EffectiveToTimeKey <= v_TimeKey
           ...
           AND NOT EXISTS ( SELECT 1 
                            FROM tt_AdvCustNPAdetail_15 BB
         ...
         FROM TEMP ,tt_AdvCustNPAdetail_15 TEMP
                JOIN ( SELECT tt_AdvCustNPAdetail_15.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvCustNPAdetail_15 
                        WHERE  tt_AdvCustNPAdetail_15.IsChanged IN ( 'N','C' )

                                 AND tt_AdvCustNPAdetail_15.EntityKey = 0
                                 OR tt_AdvCustNPAdetail_15.EntityKey IS NULL ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         FROM TEMP ,tt_NPA_DATA_4 TEMP
                JOIN ( SELECT tt_NPA_DATA_4.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) NewEntityKey  
                       FROM tt_NPA_DATA_4 
                        WHERE  tt_NPA_DATA_4.EntityKey = 0
                                 OR tt_NPA_DATA_4.EntityKey IS NULL
                                 AND tt_NPA_DATA_4.EffectiveToTimeKey > v_TimeKey ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         IF utils.object_id('TEMPDB..tt_AdvAcFinancialDetail_8') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcFinancialDetail_8 ';
         END IF;
         DELETE FROM tt_AdvAcFinancialDetail_8;
         INSERT INTO tt_AdvAcFinancialDetail_8
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcFinancialDetail O
                JOIN tt_AdvAcFinancialDetail_8 T   ON O.AccountEntityID = T.AccountEntityID
                AND O.EffectiveFromTimeKey <= v_TimeKey
                AND O.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         ------Select * 
         --from tt_AdvAcFinancialDetail_8 A
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcFinancialDetail_8 TEMP
                JOIN ( SELECT tt_AdvAcFinancialDetail_8.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcFinancialDetail_8 
                        WHERE  NVL(tt_AdvAcFinancialDetail_8.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         ---Where Temp.IsChanged in ('N','C')
         ----UPDATE A
         ...
         ----		SELECT COUNT(1), AccountEntityId FROM tt_AdvAcFinancialDetail_8 
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcFinancialDetail
           ...
           ( SELECT ENTITYKEY ,
                    ...
                    AccountBlkCode2 
             FROM tt_AdvAcFinancialDetail_8 T
         ...
         IF utils.object_id('TEMPDB..#AdvAcBalanceDetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcBalanceDetail_15 ';
         END IF;
         DELETE FROM tt_AdvAcBalanceDetail_16;
         INSERT INTO tt_AdvAcBalanceDetail_15
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcBalanceDetail O
                JOIN tt_AdvAcBalanceDetail_15 T   ON O.AccountEntityID = T.AccountEntityID
                AND o.EffectiveFromTimeKey <= v_TimeKey
                AND o.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcBalanceDetail_15 TEMP
                JOIN ( SELECT tt_AdvAcBalanceDetail_15.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcBalanceDetail_15 
                        WHERE  NVL(tt_AdvAcBalanceDetail_15.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         --Where Temp.IsChanged in ('N','C')
         /***************************************************************************************************************/
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ...
           ( SELECT EntityKey ,
                    ...
                    SourceAssetClass ,
                    SourceNpaDate 
             FROM tt_AdvAcBalanceDetail_15 T
              WHERE  NVL(T.IsChanged, 'U') NOT IN ( 'D','K' )
            );
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ( EntityKey, AccountEntityId, AssetClassAlt_Key, BalanceInCurrency, Balance, SignBalance, LastCrDt, OverDue, TotalProv, RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, OverDueSinceDt, MocStatus, MocDate, MocTypeAlt_Key, Old_OverDueSinceDt, Old_OverDue, ORG_TotalProv, IntReverseAmt, UnAppliedIntAmount, PS_Balance, NPS_Balance, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, CreatedBy, UpgradeDate, OverduePrincipal, NotionalInttAmt, PrincipalBalance, Overdueinterest, AdvanceRecovery, PS_NPS_FLAG, DFVAmt, InterestReceivable, OverduePrincipalDt, OverdueIntDt, OverOtherdue, OverdueOtherDt, SourceAssetClass, SourceNpaDate )
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21122023_BAK" TO "ADF_CDR_RBL_STGDB";
