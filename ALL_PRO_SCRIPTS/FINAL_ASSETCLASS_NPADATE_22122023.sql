--------------------------------------------------------
--  DDL for Procedure FINAL_ASSETCLASS_NPADATE_22122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" /*=========================================
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         FROM B ,tt_CustomerCal_10 A
                JOIN tt_ACCOUNTCAL_20 B   ON A.RefCustomerID = B.RefCustomerID
         ...
         ELSE ( SELECT DIMASSETCLASS.AssetClassAlt_Key 
          ...
          WHERE  DIMASSETCLASS.AssetClassShortName = 'STD'
            ...
            END AS finalAssetClassAlt_Key
         FROM B ,tt_CustomerCal_10 A
                JOIN tt_ACCOUNTCAL_20 B   ON A.SourceSystemCustomerID = B.SourceSystemCustomerID
                AND ( NVL(A.FlgProcessing, 'N') = 'N' ) 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CoBorrowerCal A
                JOIN tt_ACCOUNTCAL_20 B   ON A.CustomerACID = B.CUSTOMERACID ) src
         ON ( A.ROWID = src.row_id )
         ...
         DELETE FROM tt_MAXASSETCLASS_7;
         UTILS.IDENTITY_RESET('tt_MAXASSETCLASS_7');

         INSERT INTO tt_MAXASSETCLASS_7 ( 
         	SELECT Cohort_No ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CoBorrowerCal A
                JOIN tt_MAXASSETCLASS_7 B   ON A.Cohort_No = B.Cohort_No 
         ...
         --IF OBJECT_ID('TEMPDB..tt_TEMPTABLE_UCFIC1_12') IS NOT NULL
         --   DROP TABLE tt_TEMPTABLE_UCFIC1_12
         ...
         -- INTO tt_TEMPTABLE_UCFIC1_12 FROM PRO.CUSTOMERCAL A
         ...
         IF utils.object_id('TEMPDB..tt_CTE_PERC_11') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CTE_PERC_11 ';
         END IF;
         DELETE FROM tt_CTE_PERC_11;
         UTILS.IDENTITY_RESET('tt_CTE_PERC_11');

         INSERT INTO tt_CTE_PERC_11 ( 
         	SELECT * 
         	  FROM (  /* ADVANCE DATA */
                   SELECT UCIF_ID ,
         ...
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_UCFIC1_12') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_UCFIC1_12 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_UCFIC1_12;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_UCFIC1_12');

         INSERT INTO tt_TEMPTABLE_UCFIC1_12 ( 
         	SELECT UCIF_ID ,
         ...
         FROM A ,tt_TEMPTABLE_UCFIC1_12 A
                JOIN tt_CTE_PERC_11 B   ON A.UCIF_ID = B.UCIF_ID
         ...
         IF tt_CTE_PERC_11  --SQLDEV: NOT RECOGNIZED
         MERGE INTO A 
         ...
         ELSE A.DegReason
            END AS pos_4
         FROM A ,tt_CustomerCal_10 A
                JOIN tt_TEMPTABLE_UCFIC1_12 B   ON A.UCIF_ID = B.UCIF_ID ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,InvestmentFinancialDetail A
                ...
                AND C.EffectiveFromTimeKey <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY
                JOIN tt_TEMPTABLE_UCFIC1_12 D   ON D.UCIF_ID = C.UcifId ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,CurDat_RBL_MISDB_PROD.DerivativeDetail A
                JOIN tt_TEMPTABLE_UCFIC1_12 B   ON A.UCIC_ID = B.UCIF_ID
                AND A.EffectiveFromTimeKey <= v_TIMEKEY
         ...
         IF tt_TEMPTABLE_UCFIC1_12  --SQLDEV: NOT RECOGNIZED
         ...
         IF tt_CTE_AA_9  --SQLDEV: NOT RECOGNIZED
         tt_CTE_AA_9 TABLE IF  --SQLDEV: NOT RECOGNIZED
         IF tt_CTE_BB_9  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CTE_AA_9;
         UTILS.IDENTITY_RESET('tt_CTE_AA_9');

         INSERT INTO tt_CTE_AA_9 ( 
         	SELECT UcifEntityID 
         	  FROM tt_ACCOUNTCAL_20 A
         ...
         DELETE FROM tt_CTE_BB_9;
         UTILS.IDENTITY_RESET('tt_CTE_BB_9');

         INSERT INTO tt_CTE_BB_9 ( 
         	SELECT A.UcifEntityID 
         	  FROM tt_CTE_AA_9 A
                   JOIN tt_ACCOUNTCAL_20 B   ON A.UcifEntityID = B.UcifEntityID
         ...
         FROM B ,tt_ACCOUNTCAL_20 A
                JOIN tt_CTE_BB_9 AA   ON A.UcifEntityID = A.UcifEntityID
         ...
         FROM A ,tt_ACCOUNTCAL_20 A
                JOIN tt_CustomerCal_10 B   ON A.RefCustomerID = B.RefCustomerID
         ...
         USING (SELECT A.ROWID row_id, NVL(B.SysAssetClassAlt_Key, 1) AS pos_2, B.SysNPA_Dt
         FROM A ,tt_ACCOUNTCAL_20 A
                JOIN tt_CustomerCal_10 B   ON A.UcifEntityID = B.UcifEntityID 
          WHERE NVL(B.SysAssetClassAlt_Key, 1) <> 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.FinalAssetClassAlt_Key = pos_2,
                                      A.FinalNpaDt = src.SysNPA_Dt;
         ...
         ----FROM PRO.CustomerCal A INNER JOIN tt_TEMPTABLE_UCFIC1_12 B ON A.UCIF_ID=B.UCIF_ID
         ...
         IF utils.object_id('TEMPDB..tt_Data_12') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Data_12 ';
         END IF;
         DELETE FROM tt_Data_12;
         UTILS.IDENTITY_RESET('tt_Data_12');

         INSERT INTO tt_Data_12 ( 
         	SELECT DISTINCT DegReason ,
                          UcifEntityID 
         	  FROM tt_ACCOUNTCAL_20 
         	 WHERE  DegReason IS NOT NULL
                    AND FLGDEG = 'Y' );
         IF utils.object_id('TEMPDB..tt_NPADegReason_12') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_NPADegReason_12 ';
         END IF;
         DELETE FROM tt_NPADegReason_12;
         UTILS.IDENTITY_RESET('tt_NPADegReason_12');

         INSERT INTO tt_NPADegReason_12 ( 
         	SELECT UcifEntityID ,
         ...
         --DROP TABLE #DD
         --Select SourceSystemCustomerID ,DegReason ,ROW_NUMBER()OVER(PARTITION by SourceSystemCustomerID order by SourceSystemCustomerID) AS RN  INTO #DD  FROM tt_Data_12
         --IF object_id('TEMPDB..tt_NPADegReason_12') is NOT NULL
         --DROP TABLE tt_NPADegReason_12
         --SELECT SourceSystemCustomerID ,DegReason INTO tt_NPADegReason_12 FROM
         --(
         ...
         USING (SELECT A.ROWID row_id, B.DegReason
         FROM A ,tt_CustomerCal_10 A
                JOIN tt_NPADegReason_12 B   ON A.UcifEntityID = B.UcifEntityID
                AND A.FlgDeg = 'Y' ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET B.DegReason = src.DegReason;
         UPDATE tt_CustomerCal_10
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_22122023" TO "ADF_CDR_RBL_STGDB";
