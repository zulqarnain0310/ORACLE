--------------------------------------------------------
--  DDL for Procedure CUSTACCOUNTMERGE_MOC_21082024
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" 
(
  v_TIMEKEY IN NUMBER
)
AS
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TEMPDB..#AdvCustNPAdetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvCustNPAdetail_13 ';
         END IF;
         DELETE FROM tt_AdvCustNPAdetail_14;
         INSERT INTO tt_AdvCustNPAdetail_13
         ...
         IF tt_NPA_DATA_3  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_NPA_DATA_3;
         UTILS.IDENTITY_RESET('tt_NPA_DATA_3');

         INSERT INTO tt_NPA_DATA_3 ( 
         	SELECT A.* ,
                 UTILS.CONVERT_TO_NUMBER(0,10,0) NewEntityKey  ,
                 'N' IsChanged  
         	  FROM AdvCustNPADetail A
                   JOIN tt_CUSTOMERCAL_12 B   ON A.CustomerEntityId = B.CustomerEntityId
         	 WHERE  A.EffectiveFromTimeKey <= v_TimeKey
                    AND A.EffectiveToTimeKey >= v_TimeKey );
         ----------For New Records
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'N'
         FROM A ,tt_AdvCustNPAdetail_13 A 
          WHERE NOT EXISTS ( SELECT 1 
                             FROM tt_NPA_DATA_3 B
         ...
         FROM O ,RBL_MISDB_PROD.AdvCustNPADetail O
                JOIN tt_AdvCustNPAdetail_13 T   ON O.CustomerEntityId = T.CustomerEntityId
         ...
         FROM AA ,RBL_MISDB_PROD.AdvCustNPADetail AA 
          WHERE AA.EffectiveToTimeKey <= v_TimeKey
           ...
           AND NOT EXISTS ( SELECT 1 
                            FROM tt_AdvCustNPAdetail_13 BB
         ...
         FROM TEMP ,tt_AdvCustNPAdetail_13 TEMP
                JOIN ( SELECT tt_AdvCustNPAdetail_13.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvCustNPAdetail_13 
                        WHERE  tt_AdvCustNPAdetail_13.IsChanged IN ( 'N','C' )

                                 AND tt_AdvCustNPAdetail_13.EntityKey = 0
                                 OR tt_AdvCustNPAdetail_13.EntityKey IS NULL ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         FROM TEMP ,tt_NPA_DATA_3 TEMP
                JOIN ( SELECT tt_NPA_DATA_3.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) NewEntityKey  
                       FROM tt_NPA_DATA_3 
                        WHERE  tt_NPA_DATA_3.EntityKey = 0
                                 OR tt_NPA_DATA_3.EntityKey IS NULL
                                 AND tt_NPA_DATA_3.EffectiveToTimeKey > v_TimeKey ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         IF utils.object_id('TEMPDB..tt_AdvAcFinancialDetail_7') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcFinancialDetail_7 ';
         END IF;
         DELETE FROM tt_AdvAcFinancialDetail_7;
         INSERT INTO tt_AdvAcFinancialDetail_7
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcFinancialDetail O
                JOIN tt_AdvAcFinancialDetail_7 T   ON O.AccountEntityID = T.AccountEntityID
                AND O.EffectiveFromTimeKey <= v_TimeKey
                AND O.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         ------Select * 
         --from tt_AdvAcFinancialDetail_7 A
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcFinancialDetail_7 TEMP
                JOIN ( SELECT tt_AdvAcFinancialDetail_7.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcFinancialDetail_7 
                        WHERE  NVL(tt_AdvAcFinancialDetail_7.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         ---Where Temp.IsChanged in ('N','C')
         ----UPDATE A
         ...
         ----		SELECT COUNT(1), AccountEntityId FROM tt_AdvAcFinancialDetail_7 
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcFinancialDetail
           ...
           ( SELECT ENTITYKEY ,
                    ...
                    AccountBlkCode2 
             FROM tt_AdvAcFinancialDetail_7 T
         ...
         IF utils.object_id('TEMPDB..#AdvAcBalanceDetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcBalanceDetail_13 ';
         END IF;
         DELETE FROM tt_AdvAcBalanceDetail_14;
         INSERT INTO tt_AdvAcBalanceDetail_13
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcBalanceDetail O
                JOIN tt_AdvAcBalanceDetail_13 T   ON O.AccountEntityID = T.AccountEntityID
                AND o.EffectiveFromTimeKey <= v_TimeKey
                AND o.EffectiveToTimeKey >= v_TimeKey
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcBalanceDetail_13 TEMP
                JOIN ( SELECT tt_AdvAcBalanceDetail_13.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcBalanceDetail_13 
                        WHERE  NVL(tt_AdvAcBalanceDetail_13.IsChanged, 'U') <> 'D' ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         --Where Temp.IsChanged in ('N','C')
         /***************************************************************************************************************/
         ...
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ...
           ( SELECT EntityKey ,
                    ...
                    SourceAssetClass ,
                    SourceNpaDate 
             FROM tt_AdvAcBalanceDetail_13 T
              WHERE  NVL(T.IsChanged, 'U') NOT IN ( 'D','K' )
            );
         INSERT INTO RBL_MISDB_PROD.AdvAcBalanceDetail
           ( EntityKey, AccountEntityId, AssetClassAlt_Key, BalanceInCurrency, Balance, SignBalance, LastCrDt, OverDue, TotalProv, RefCustomerId, RefSystemAcId, AuthorisationStatus, EffectiveFromTimeKey, EffectiveToTimeKey, OverDueSinceDt, MocStatus, MocDate, MocTypeAlt_Key, Old_OverDueSinceDt, Old_OverDue, ORG_TotalProv, IntReverseAmt, UnAppliedIntAmount, PS_Balance, NPS_Balance, DateCreated, ModifiedBy, DateModified, ApprovedBy, DateApproved, CreatedBy, UpgradeDate, OverduePrincipal, NotionalInttAmt, PrincipalBalance, Overdueinterest, AdvanceRecovery, PS_NPS_FLAG, DFVAmt, InterestReceivable, OverduePrincipalDt, OverdueIntDt, OverOtherdue, OverdueOtherDt, SourceAssetClass, SourceNpaDate )
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."CUSTACCOUNTMERGE_MOC_21082024" TO "ADF_CDR_RBL_STGDB";
