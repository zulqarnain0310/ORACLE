--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" /*=========================================
      ...
      BEGIN
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL
           ...
           ( SELECT ACCOUNTENTITYID ACCOUNTENTITYID  ,
                    ...
                    v_TIMEKEY EFFECTIVETOTIMEKEY  ,
                    CASE 
                         WHEN ( NVL(ABD.ISLAD, 0) = 1
                           OR ABD.assetclass = '1' ) THEN 'ALWYS_STD'
                         WHEN ABD.assetclass = '2' THEN 'ALWYS_NPA'
                    ELSE 'NORMAL'
                       END ASSET_NORM  ,
                    0 SPLCATG1ALT_KEY  ,
                    0 SPLCATG2ALT_KEY  ,
         ...
         /*  amar 02112021 -  product code include in account cal insert
         ...
         */
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
               ...
               FROM DUAL  );
         ---Condition Change Required Modification Done ---- 
         -- COMMENEDT BY AMAR ON 12-06-2021 BANK IS PROVIDING IN SOURCE DATA
         MERGE INTO A 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_36') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_36 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_36;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_36');

            INSERT INTO tt_TEMPTABLEPANCARD_36 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_36 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_36 A
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_36 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_36 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_36') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_36 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_36;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_36');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_36 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_36 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_36 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         WHEN StatusDate IS NULL THEN StatusDate
         ...
         WHEN MATCHED THEN UPDATE SET A.Asset_Norm = 'ALWYS_NPA',
                                      ...
                                      A.SysNPA_Dt = pos_5;
         MERGE INTO A 
         USING (SELECT A.ROWID row_id, 'ALWYS_NPA', 'NPA DUE TO WRITEOFF MARKING' AS pos_3, CASE 
         WHEN StatusDate IS NULL THEN StatusDate
         ELSE v_ProcessingDate
            END AS pos_4
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
               ...
               FROM DUAL  );
         ---Condition Change Required  Modification Done--- 
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_36') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_36 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_36;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_36');

         INSERT INTO tt_PRVQTRRV_36 ( 
         	SELECT * 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_36 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_36') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_36 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_36;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_36');

         INSERT INTO tt_CurntQtrRv_36 ( 
         	SELECT * 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_36 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_36') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_36 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_36;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_36');

         INSERT INTO tt_BILL_OVERDUE_36 ( 
         ...
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_36_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_FINAL_34 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_FINAL_34;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_FINAL_34');

         INSERT INTO tt_BILL_OVERDUE_FINAL_34 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  BILOVERDUE  ,
                 MIN(BILLDUEDT)  BILLOVERDUEDT  
         	  FROM tt_BILL_OVERDUE_36 
         	  GROUP BY AccountEntityId );
         ...
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_FINAL_34 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_36') IS NOT NULL THEN
          --update a
         ...
         /*RESTRUCTURE UPDATES */
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_36 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_36;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_36');

         INSERT INTO tt_PC_OVERDUE_36 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ELSE PCExtendedDueDt
                     END) < v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_36_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_36 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_36;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_36');

         INSERT INTO tt_PC_OVERDUE_FINAL_36 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_36 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_36 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_36') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_36 ';
         END IF;
         DELETE FROM tt_Stock_36;
         UTILS.IDENTITY_RESET('tt_Stock_36');

         INSERT INTO tt_Stock_36 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_36_IX ON tt_Stock_36(AccountEntityId)
         ----CREATE NONCLUSTERED INDEX tt_Stock_36NON ON tt_Stock_36(AccountEntityId,StkSmtDt)
         ----IF OBJECT_ID('Tempdb..tt_Stock_362') IS NOT NULL
         ----DROP TABLE tt_Stock_362
         ...
         ----			GROUP BY AccountEntityId
         ----CREATE CLUSTERED INDEX tt_Stock_362_IX ON tt_Stock_362(AccountEntityId)
         ----CREATE NONCLUSTERED INDEX tt_Stock_362NON ON tt_Stock_362(AccountEntityId,StkSmtDt)
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A 
         --INNER  JOIN tt_Stock_36 SD ON A.AccountEntityId=SD.AccountEntityId
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_71') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_71 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_71;
         INSERT INTO tt_TEMPDerecognisedInterest_71
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_71 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_72') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_72 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_72;
         INSERT INTO tt_TEMPDerecognisedInterest_72
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_72 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         /*
         INSERT INTO PRO.ProcessMonitor(UserID,Description,MODE,StartTime,EndTime,TimeKey,SetID)
         SELECT ORIGINAL_LOGIN(),'UPDATE UnserviedInt FROM AdvAcFinancialDetail','RUNNING',GETDATE(),NULL,@TIMEKEY,@SetID
         ...
         */
         ...
         IF tt_MOC_DATA_36  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_36;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_36');

         INSERT INTO tt_MOC_DATA_36 ( 
         	SELECT UcifEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_36 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_OPT_04122023" TO "ADF_CDR_RBL_STGDB";
