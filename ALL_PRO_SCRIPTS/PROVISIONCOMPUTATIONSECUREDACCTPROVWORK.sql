...
BEGIN
   ...
   /* DPS BASE PROVISION */
   IF tt_AcctProvWork  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_AcctProvWork;
   UTILS.IDENTITY_RESET('tt_AcctProvWork');

   INSERT INTO tt_AcctProvWork ( 
   ...
   /* UPDATE tt_AcctProvWork MAX ON CUSTOMER LEVEL  01-06-2021 */
   MERGE INTO A 
   USING (SELECT A.ROWID row_id, B.LastCrDt
   FROM A ,tt_AcctProvWork A
          JOIN ( SELECT tt_AcctProvWork.CustomerEntityID ,
                        MAX(tt_AcctProvWork.LastCrDt)  LastCrDt  
                 FROM tt_AcctProvWork 
                   GROUP BY tt_AcctProvWork.CustomerEntityID ) B   ON A.CustomerEntityID = B.CustomerEntityID ) src
   ...
   FROM A ,tt_AcctProvWork a ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET A.HighestOverdueProv = src.HighestOverdueProv;
   --select * FROM tt_AcctProvWork where AccountEntityID=2 
   ...
   FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
          JOIN tt_AcctProvWork B   ON A.AccountEntityID = B.AccountEntityID 
   ...
   FROM A ,tt_AcctProvWork A
          JOIN ( SELECT tt_AcctProvWork.UcifEntityID ,
                        MAX(tt_AcctProvWork.FinalProv)  FinalProv  
                 FROM tt_AcctProvWork 
                   GROUP BY tt_AcctProvWork.UcifEntityID ) B   ON A.UcifEntityID = B.UcifEntityID ) src
   ON ( A.ROWID = src.row_id )
   ...
   USING (SELECT A.ROWID row_id, B.FinalProv
   FROM A ,tt_AcctProvWork A
          JOIN ( SELECT tt_AcctProvWork.PANNO ,
                        MAX(tt_AcctProvWork.FinalProv)  FinalProv  
                 FROM tt_AcctProvWork 
                  WHERE  tt_AcctProvWork.PANNO IS NOT NULL
                   GROUP BY tt_AcctProvWork.PANNO ) B   ON A.PANNO = B.PANNO ) src
   ON ( A.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET A.FinalProv = src.FinalProv;
