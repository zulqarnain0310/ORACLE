--------------------------------------------------------
--  DDL for Procedure FINAL_ASSETCLASS_NPADATE_TEST_04122023
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" /*=========================================
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         ----FROM PRO.CUSTOMERCAL_test  A INNER JOIN PRO.ACCOUNTCAL_test B ON A.SourceSystemCustomerID=B.SourceSystemCustomerID AND (A.FlgProcessing='N')
         ...
         FROM B ,PRO_RBL_MISDB_PROD.CUSTOMERCAL_test A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL_TEST B   ON A.RefCustomerID = B.RefCustomerID
         ...
         ELSE ( SELECT DIMASSETCLASS.AssetClassAlt_Key 
          ...
          WHERE  DIMASSETCLASS.AssetClassShortName = 'STD'
            ...
            END AS finalAssetClassAlt_Key
         FROM B ,PRO_RBL_MISDB_PROD.CUSTOMERCAL_test A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL_TEST B   ON A.SourceSystemCustomerID = B.SourceSystemCustomerID
                AND ( NVL(A.FlgProcessing, 'N') = 'N' ) 
         ...
         --IF OBJECT_ID('TEMPDB..tt_TEMPTABLE_UCFIC1_20') IS NOT NULL
         --   DROP TABLE tt_TEMPTABLE_UCFIC1_20
         ...
         -- INTO tt_TEMPTABLE_UCFIC1_20 FROM PRO.CUSTOMERCAL_test A
         ...
         IF utils.object_id('TEMPDB..tt_CTE_PERC_19') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CTE_PERC_19 ';
         END IF;
         DELETE FROM tt_CTE_PERC_19;
         UTILS.IDENTITY_RESET('tt_CTE_PERC_19');

         INSERT INTO tt_CTE_PERC_19 ( 
         	SELECT * 
         	  FROM (  /* ADVANCE DATA */
                   SELECT UCIF_ID ,
         ...
         IF utils.object_id('TEMPDB..tt_TEMPTABLE_UCFIC1_20') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_UCFIC1_20 ';
         END IF;
         DELETE FROM tt_TEMPTABLE_UCFIC1_20;
         UTILS.IDENTITY_RESET('tt_TEMPTABLE_UCFIC1_20');

         INSERT INTO tt_TEMPTABLE_UCFIC1_20 ( 
         	SELECT UCIF_ID ,
         ...
         FROM A ,tt_TEMPTABLE_UCFIC1_20 A
                JOIN tt_CTE_PERC_19 B   ON A.UCIF_ID = B.UCIF_ID
         ...
         IF tt_CTE_PERC_19  --SQLDEV: NOT RECOGNIZED
         MERGE INTO A 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL_test A
                JOIN tt_TEMPTABLE_UCFIC1_20 B   ON A.UCIF_ID = B.UCIF_ID ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,InvestmentFinancialDetail A
                ...
                AND C.EffectiveFromTimeKey <= v_TIMEKEY
                AND C.EffectiveToTimeKey >= v_TIMEKEY
                JOIN tt_TEMPTABLE_UCFIC1_20 D   ON D.UCIF_ID = C.UcifId ) src
         ON ( A.ROWID = src.row_id )
         ...
         FROM A ,CurDat_RBL_MISDB_PROD.DerivativeDetail A
                JOIN tt_TEMPTABLE_UCFIC1_20 B   ON A.UCIC_ID = B.UCIF_ID
                AND A.EffectiveFromTimeKey <= v_TIMEKEY
         ...
         IF tt_TEMPTABLE_UCFIC1_20  --SQLDEV: NOT RECOGNIZED
         MERGE INTO A 
         ...
         ----FROM PRO.CUSTOMERCAL_test A INNER JOIN tt_TEMPTABLE_UCFIC1_20 B ON A.UCIF_ID=B.UCIF_ID
         ...
         ---- INTO #TEMPTABLE_UCFICDbtDt FROM PRO.CUSTOMERCAL_test
         ...
         ----INNER JOIN PRO.CUSTOMERCAL_test B ON A.UcifEntityID=B.UcifEntityID	 AND B.DbtDt IS NULL
         MERGE INTO B 
         USING (SELECT B.ROWID row_id, A.SysNPA_Dt
         FROM B ,PRO_RBL_MISDB_PROD.CUSTOMERCAL_test A
                JOIN PRO_RBL_MISDB_PROD.ACCOUNTCAL_TEST B   ON A.SourceSystemCustomerID = B.SourceSystemCustomerID 
         ...
         WHEN MATCHED THEN UPDATE SET A.FinalAssetClassAlt_Key = 1,
                                      A.FinalNpaDt = NULL,
                                      A.DEGREASON = NULL;
         ---UPDATE  MULTIPLE   DegReason IN PRO.CUSTOMERCAL_test TABLE-------
         IF utils.object_id('TEMPDB..tt_Data_20') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Data_20 ';
         END IF;
         DELETE FROM tt_Data_20;
         UTILS.IDENTITY_RESET('tt_Data_20');

         INSERT INTO tt_Data_20 ( 
         	SELECT DISTINCT DegReason ,
                          UcifEntityID 
         	  FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL_TEST 
         	 WHERE  DegReason IS NOT NULL
                    AND FLGDEG = 'Y' );
         IF utils.object_id('TEMPDB..tt_NPADegReason_20') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_NPADegReason_20 ';
         END IF;
         DELETE FROM tt_NPADegReason_20;
         UTILS.IDENTITY_RESET('tt_NPADegReason_20');

         INSERT INTO tt_NPADegReason_20 ( 
         	SELECT UcifEntityID ,
         ...
         --DROP TABLE #DD
         --Select SourceSystemCustomerID ,DegReason ,ROW_NUMBER()OVER(PARTITION by SourceSystemCustomerID order by SourceSystemCustomerID) AS RN  INTO #DD  FROM tt_Data_20
         --IF object_id('TEMPDB..tt_NPADegReason_20') is NOT NULL
         --DROP TABLE tt_NPADegReason_20
         --SELECT SourceSystemCustomerID ,DegReason INTO tt_NPADegReason_20 FROM
         --(
         ...
         USING (SELECT A.ROWID row_id, B.DegReason
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL_test A
                JOIN tt_NPADegReason_20 B   ON A.UcifEntityID = B.UcifEntityID
                AND A.FlgDeg = 'Y' ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET B.DegReason = src.DegReason;
         UPDATE PRO_RBL_MISDB_PROD.CUSTOMERCAL_test
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."FINAL_ASSETCLASS_NPADATE_TEST_04122023" TO "ADF_CDR_RBL_STGDB";
