--------------------------------------------------------
--  DDL for Procedure INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" /*=========================================  
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLEPANCARD_30') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLEPANCARD_30 ';
            END IF;
            DELETE FROM tt_TEMPTABLEPANCARD_30;
            UTILS.IDENTITY_RESET('tt_TEMPTABLEPANCARD_30');

            INSERT INTO tt_TEMPTABLEPANCARD_30 ( 
            	SELECT PANNO ,
            ...
            EXECUTE IMMEDIATE ' ALTER TABLE tt_TEMPTABLEPANCARD_30 
               ADD ( SOURCEDBNAME VARCHAR2(20)  ) ';
            MERGE INTO A 
            USING (SELECT A.ROWID row_id, C.SOURCEDBNAME
            FROM A ,tt_TEMPTABLEPANCARD_30 A
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_30 B   ON A.PANNO = B.PANNO ) src
            ON ( A.ROWID = src.row_id )
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLEPANCARD_30 B   ON A.PANNO = B.PANNO 
         ...
         BEGIN
            IF utils.object_id('TEMPDB..tt_TEMPTABLE_ADHARCARD_30') IS NOT NULL THEN
             EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPTABLE_ADHARCARD_30 ';
            END IF;
            DELETE FROM tt_TEMPTABLE_ADHARCARD_30;
            UTILS.IDENTITY_RESET('tt_TEMPTABLE_ADHARCARD_30');

            INSERT INTO tt_TEMPTABLE_ADHARCARD_30 ( 
            	SELECT AADHARCARDNO ,
                    MAX(NVL(SYSASSETCLASSALT_KEY, 1))  SYSASSETCLASSALT_KEY  ,
                    MIN(SYSNPA_DT)  SYSNPA_DT  ,
                    B.SOURCEDBNAME 
            ...
            USING (SELECT A.ROWID row_id, B.SYSASSETCLASSALT_KEY, B.SYSNPA_DT
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_30 B   ON A.AadharCardNO = B.AADHARCARDNO ) src
            ON ( A.ROWID = src.row_id )
            WHEN MATCHED THEN UPDATE SET A.SYSASSETCLASSALT_KEY = src.SYSASSETCLASSALT_KEY,
                                         A.SYSNPA_DT = src.SYSNPA_DT;
            MERGE INTO A 
            ...
            FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                   JOIN tt_TEMPTABLE_ADHARCARD_30 B   ON A.AadharCardNO = B.AADHARCARDNO 
             WHERE A.SrcAssetClassAlt_Key = 1
              AND A.SysAssetClassAlt_Key > 1
              AND A.DegReason IS NULL) src
            ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         --IF OBJECT_ID('TEMPDB..tt_PRVQTRRV_30') IS NOT NULL  
         --DROP TABLE tt_PRVQTRRV_30  
         ...
         --FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_30 B  
         ...
         IF utils.object_id('TEMPDB..tt_PRVQTRRV_30') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PRVQTRRV_30 ';
         END IF;
         DELETE FROM tt_PRVQTRRV_30;
         UTILS.IDENTITY_RESET('tt_PRVQTRRV_30');

         INSERT INTO tt_PRVQTRRV_30 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_PRVQTRRV_30 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --UPDATE A SET A.PRVQTRRV= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)  FROM PRO.CUSTOMERCAL A INNER  JOIN tt_PRVQTRRV_30 B                  
         --ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         IF utils.object_id('TEMPDB..tt_CurntQtrRv_30') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_CurntQtrRv_30 ';
         END IF;
         DELETE FROM tt_CurntQtrRv_30;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_30');

         INSERT INTO tt_CurntQtrRv_30 ( 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_30 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key <> 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = CurrentValue;
         ------- ----Added By Mandeep (Security Perfection)
         DELETE FROM tt_CurntQtrRv_Finacle_23;
         UTILS.IDENTITY_RESET('tt_CurntQtrRv_Finacle_23');

         INSERT INTO tt_CurntQtrRv_Finacle_23 ( 
         	SELECT CustomerEntityId ,
                 SUM(b.CurrentValue)  CurrentValue  
         	  FROM AdvSecurityDetail A
                   JOIN AdvSecurityValueDetail B   ON A.SecurityEntityID = B.SecurityEntityID
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_CurntQtrRv_Finacle_23 B   ON A.CustomerEntityID = B.CUSTOMERENTITYID 
          WHERE A.SourceAlt_Key = 1) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.CurntQtrRv = src.CurrentValue;
         /*                  
         --IF OBJECT_ID('TEMPDB..tt_CurntQtrRv_30') IS NOT NULL                  
         --DROP TABLE tt_CurntQtrRv_30                  

         --SELECT *                    
         --INTO tt_CurntQtrRv_30                   
         --FROM dbo.AdvCustSecurityFun(@TIMEKEY)                  

         ------UPDATE A SET A.CurntQtrRv= ISNULL(B.Total_PriSec,0)+ISNULL(B.Total_CollSec,0)                   
         ------FROM PRO.CUSTOMERCAL A INNER  JOIN tt_CurntQtrRv_30 B                  
         ------ON A.CUSTOMERENTITYID=B.CUSTOMERENTITYID                  
         */
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
         ...
         */
         ...
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = NULL,
                                      ...
                                      OverdueAmt = 0,
                                      OtherOverdue = 0;
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_30') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_30 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_30;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_30');

         INSERT INTO tt_BILL_OVERDUE_30 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_30_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_FINAL_28 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_FINAL_28;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_FINAL_28');

         INSERT INTO tt_BILL_OVERDUE_FINAL_28 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  BILOVERDUE  ,
                 MIN(BILLDUEDT)  BILLOVERDUEDT  
         	  FROM tt_BILL_OVERDUE_30 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.BILLOVERDUEDT, B.BILOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_FINAL_28 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET OVERDUESINCEDT = src.BILLOVERDUEDT,
                                      OverdueAmt = src.BILOVERDUE;
         --=====SCF SOURCE SYSTEM CHANGES==================================================================================================
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_30_SCF') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_26 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_26;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_26');

         INSERT INTO tt_BILL_OVERDUE_SCF_26 ( 
         	SELECT AccountEntityID ,
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
         	  ...
         	  GROUP BY AccountEntityId );
         IF utils.object_id('TEMPDB..tt_BILL_OVERDUE_30_SCF_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_OVERDUE_SCF_FINAL_26 ';
         END IF;
         DELETE FROM tt_BILL_OVERDUE_SCF_FINAL_26;
         UTILS.IDENTITY_RESET('tt_BILL_OVERDUE_SCF_FINAL_26');

         INSERT INTO tt_BILL_OVERDUE_SCF_FINAL_26 ( 
         	SELECT AccountEntityId ,
                 Balance ,
                 overdueinterest ,
         ...
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_OVERDUE_SCF_FINAL_26 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET PrincOverdueSinceDt = src.BillDueDt,
                                      ...
                                      PrincOverdue = pos_6,
                                      IntOverdue = pos_7;
         IF utils.object_id('TEMPDB..tt_BILL_Min_ReviewDt_26') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_BILL_Min_ReviewDt_26 ';
         END IF;
         DELETE FROM tt_BILL_Min_ReviewDt_26;
         UTILS.IDENTITY_RESET('tt_BILL_Min_ReviewDt_26');

         INSERT INTO tt_BILL_Min_ReviewDt_26 ( 
         	SELECT AccountEntityId ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_BILL_Min_ReviewDt_26 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( A.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET A.ReviewDueDt = ReviewDuedate;
         --=====================================================SCF BILL END===========================================================================
         --IF OBJECT_ID('TEMPDB..tt_BILL_OVERDUE_30_FINAL') IS NOT NULL  
         --    DROP TABLE tt_BILL_OVERDUE_30_FINAL  
         --SELECT AccountEntityId,SUM(BALANCE) BILOVERDUE,MIN(BILLDUEDT) BILLOVERDUEDT  
         -- INTO tt_BILL_OVERDUE_30_FINAL  
         --FROM tt_BILL_OVERDUE_30 GROUP BY AccountEntityId  
         --UPDATE PRO.ACCOUNTCAL SET OVERDUESINCEDT=B.BILLOVERDUEDT ,OverdueAmt  =B.BILOVERDUE  
         --FROM  PRO.ACCOUNTCAL A   
         --INNER JOIN tt_BILL_OVERDUE_30_FINAL B  
         -- ON A.AccountEntityID =B.AccountEntityId  
         ...
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_30') IS NOT NULL THEN
          --update a  
         ...
         /* START OF PUI WORK*/
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_30 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_30;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_30');

         INSERT INTO tt_PC_OVERDUE_30 ( 
         	SELECT AccountEntityID ,
                 PCRefNo ,
                 BALANCE ,
                 CASE 
         	 ...
         	 WHERE  EFFECTIVEFROMTIMEKEY <= v_TIMEKEY
                  ...
                  ----- )<=@PROCESSINGDATE  --  as discussed with Triloki Sir for SMA KIssue - Consider 1 dpd on due date  
                  v_PROCESSINGDATE );
         IF utils.object_id('TEMPDB..tt_PC_OVERDUE_30_FINAL') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_PC_OVERDUE_FINAL_30 ';
         END IF;
         DELETE FROM tt_PC_OVERDUE_FINAL_30;
         UTILS.IDENTITY_RESET('tt_PC_OVERDUE_FINAL_30');

         INSERT INTO tt_PC_OVERDUE_FINAL_30 ( 
         	SELECT AccountEntityId ,
                 SUM(BALANCE)  PCOVERDUE  ,
                 MIN(PCOVERDUEDUEDT)  PCOVERDUEDUEDT  
         	  FROM tt_PC_OVERDUE_30 
         	  GROUP BY AccountEntityId );
         MERGE INTO PRO_RBL_MISDB_PROD.ACCOUNTCAL 
         USING (SELECT PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID row_id, B.PCOVERDUEDUEDT, B.PCOVERDUE
         FROM PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_PC_OVERDUE_FINAL_30 B   ON A.AccountEntityID = B.AccountEntityId ) src
         ON ( PRO_RBL_MISDB_PROD.ACCOUNTCAL.ROWID = src.row_id )
         ...
         ------------END  
         -----------------------------------------------------------------------------------------------------------------  
         --DROP TABLE IF EXISTS tt_CCOD_90DAYS_INTT_CR_AMT_26  
         --SELECT A.CustomerAcID,  
         ...
         -- INTO tt_CCOD_90DAYS_INTT_CR_AMT_26  
         ...
         IF tt_CCOD_90DAYS_INTT_CR_AMT_26  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_CCOD_90DAYS_INTT_CR_AMT_26;
         UTILS.IDENTITY_RESET('tt_CCOD_90DAYS_INTT_CR_AMT_26');

         INSERT INTO tt_CCOD_90DAYS_INTT_CR_AMT_26 ( 
         	SELECT A.CustomerAcID ,
                 SUM(CASE 
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL a
                LEFT JOIN tt_CCOD_90DAYS_INTT_CR_AMT_26 b   ON A.CustomerAcID = b.CustomerAcID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('Tempdb..tt_Stock_30') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_Stock_30 ';
         END IF;
         DELETE FROM tt_Stock_30;
         UTILS.IDENTITY_RESET('tt_Stock_30');

         INSERT INTO tt_Stock_30 ( 
         	SELECT AccountEntityId ,
         	  ...
         	  GROUP BY AccountEntityId );
         ----CREATE CLUSTERED INDEX tt_Stock_30_IX ON tt_Stock_30(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_30NON ON tt_Stock_30(AccountEntityId,StkSmtDt)  
         ----IF OBJECT_ID('Tempdb..tt_Stock_302') IS NOT NULL  
         ----DROP TABLE tt_Stock_302  
         ...
         ----   GROUP BY AccountEntityId  
         ----CREATE CLUSTERED INDEX tt_Stock_302_IX ON tt_Stock_302(AccountEntityId)  
         ----CREATE NONCLUSTERED INDEX tt_Stock_302NON ON tt_Stock_302(AccountEntityId,StkSmtDt)  
         UPDATE PRO_RBL_MISDB_PROD.ProcessMonitor
            SET ENDTIME = SYSDATE,
                MODE_ = 'COMPLETE'
          WHERE  IDENTITYKEY = ( SELECT /*TODO:SQLDEV*/ IDENT_CURRENT('PRO.PROCESSMONITOR') /*END:SQLDEV*/ 
         ...
         --FROM PRO.AccountCal A   
         --INNER  JOIN tt_Stock_30 SD ON A.AccountEntityId=SD.AccountEntityId  
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_59') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_59 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_59;
         INSERT INTO tt_TEMPDerecognisedInterest_59
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_59 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         INSERT INTO PRO_RBL_MISDB_PROD.ProcessMonitor
           ...
           ( SELECT USER ,
                    ...
                    v_SetID 
               FROM DUAL  );
         IF utils.object_id('TEMPDB..tt_TEMPDerecognisedInterest_60') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TEMPDerecognisedInterest_60 ';
         END IF;
         DELETE FROM tt_TEMPDerecognisedInterest_60;
         INSERT INTO tt_TEMPDerecognisedInterest_60
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_TEMPDerecognisedInterest_60 B   ON A.AccountEntityID = B.ACCOUNTENTITYID ) src
         ON ( A.ROWID = src.row_id )
         ...
         */
         ...
         IF  --SQLDEV: NOT RECOGNIZED
         IF tt_MOC_DATA_30  --SQLDEV: NOT RECOGNIZED
         DELETE FROM tt_MOC_DATA_30;
         UTILS.IDENTITY_RESET('tt_MOC_DATA_30');

         INSERT INTO tt_MOC_DATA_30 ( 
         	SELECT UcifEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.CUSTOMERCAL A
                JOIN tt_MOC_DATA_30 B   ON A.UcifEntityID = B.UcifEntityID ) src
         ON ( A.ROWID = src.row_id )
         ...
         --IF OBJECT_ID('TEMPDB..tt_UpdateTempAssetclassAsse_26') IS NOT NULL  
         -- DROP TABLE  tt_UpdateTempAssetclassAsse_26  
         --select a.CustomerEntityID, count(a.CustomerAcID) CNT
         --into          tt_UpdateTempAssetclassAsse_26
         --from		  pro.accountcal a
         ...
         --INNER JOIN	   pro.ACCOUNTCAL  B
         --ON             A.CustomerEntityID=B.CustomerEntityID
         --INNER JOIN     tt_UpdateTempAssetclassAsse_26 C
         --ON             A.CustomerEntityID=C.CustomerEntityID
         ...
         IF utils.object_id('TEMPDB..tt_UpdateTempAssetclassAsse_26') IS NOT NULL THEN
          --------------------------COBORROWER WORK ADDED BY MANDEEP -------------------------------------------------------------------------
         EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_UpdateTempAssetclassAsse_26 ';
         END IF;
         DELETE FROM tt_UpdateTempAssetclassAsse_26;
         UTILS.IDENTITY_RESET('tt_UpdateTempAssetclassAsse_26');

         INSERT INTO tt_UpdateTempAssetclassAsse_26 ( 
         	SELECT a.CustomerEntityID ,
         ...
         FROM A ,PRO_RBL_MISDB_PROD.ACCOUNTCAL A
                JOIN tt_UpdateTempAssetclassAsse_26 C   ON A.CustomerEntityID = C.CustomerEntityID 
...
/
  ...
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ROLE_LOCAL_RBL_MISDB_PROD_ORACLE";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ETL_TEMP_RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "RBL_TEMPDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "MAIN_PRO"."INSERTDATAFORASSETCLASSFICATIONRBL_NOP_MANDEEP_2" TO "ADF_CDR_RBL_STGDB";
