   ...
   BEGIN
      ...
      BEGIN
         ...
         IF utils.object_id('TEMPDB..#AdvCustNPAdetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvCustNPAdetail ';
         END IF;
         DELETE FROM tt_AdvCustNPAdetail_2;
         ...
         FROM O ,RBL_MISDB_PROD.AdvCustNPADetail O
                JOIN tt_AdvCustNPAdetail T   ON O.CustomerEntityId = T.CustomerEntityId
         ...
         FROM TEMP ,tt_AdvCustNPAdetail TEMP
                JOIN ( SELECT tt_AdvCustNPAdetail.CustomerEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvCustNPAdetail 
                        WHERE  tt_AdvCustNPAdetail.EntityKey = 0
                                 OR tt_AdvCustNPAdetail.EntityKey IS NULL ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId 
         ...
         INSERT INTO RBL_MISDB_PROD.AdvCustNPADetail
           ...
           ( SELECT ENTITYKEY ,
                    ...
                    MocTypeAlt_Key ,
                    NPA_Reason 
             FROM tt_AdvCustNPAdetail T
              WHERE  NVL(T.IsChanged, 'U') IN ( 'N','C' )
            );
         ------------------End
         /*        AdvAcFinancialDetail  Start         */
         IF utils.object_id('TEMPDB..tt_AdvAcFinancialDetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcFinancialDetail ';
         END IF;
         DELETE FROM tt_AdvAcFinancialDetail;
         INSERT INTO tt_AdvAcFinancialDetail
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcFinancialDetail O
                JOIN tt_AdvAcFinancialDetail T   ON O.AccountEntityID = T.AccountEntityID
                AND O.EffectiveToTimeKey = 49999
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         ------Select * 
         --from tt_AdvAcFinancialDetail A
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcFinancialDetail TEMP
                JOIN ( SELECT tt_AdvAcFinancialDetail.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcFinancialDetail 
                        WHERE  tt_AdvAcFinancialDetail.EntityKey = 0
                                 OR tt_AdvAcFinancialDetail.EntityKey IS NULL ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         ---Where Temp.IsChanged in ('N','C')
         ------------------------------------------------------------------
         INSERT INTO RBL_MISDB_PROD.AdvAcFinancialDetail
           ...
           ( SELECT ENTITYKEY ,
                    ...
                    AccountBlkCode2 
             FROM tt_AdvAcFinancialDetail T
         ...
         IF utils.object_id('TEMPDB..#AdvAcBalanceDetail') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_AdvAcBalanceDetail ';
         END IF;
         DELETE FROM tt_AdvAcBalanceDetail_2;
         INSERT INTO tt_AdvAcBalanceDetail
         ...
         FROM O ,RBL_MISDB_PROD.AdvAcBalanceDetail O
                JOIN tt_AdvAcBalanceDetail T   ON O.AccountEntityID = T.AccountEntityID
                AND O.EffectiveToTimeKey = 49999
                AND O.EffectiveFromTimeKey < v_TimeKey 
         ...
         MERGE INTO TEMP 
         USING (SELECT TEMP.ROWID row_id, ACCT.EntityKey
         FROM TEMP ,tt_AdvAcBalanceDetail TEMP
                JOIN ( SELECT tt_AdvAcBalanceDetail.AccountEntityId ,
                              (v_EntityKey + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                              FROM DUAL  )  )) EntityKey  
                       FROM tt_AdvAcBalanceDetail 
                        WHERE  tt_AdvAcBalanceDetail.EntityKey = 0
                                 OR tt_AdvAcBalanceDetail.EntityKey IS NULL ) ACCT   ON TEMP.AccountEntityId = ACCT.AccountEntityId ) src
         ON ( TEMP.ROWID = src.row_id )
         WHEN MATCHED THEN UPDATE SET TEMP.EntityKey = src.EntityKey;
         --Where Temp.IsChanged in ('N','C')
         /***************************************************************************************************************/
