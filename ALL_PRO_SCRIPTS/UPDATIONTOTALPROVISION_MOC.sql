   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT Timekey - 1 
           ...
           FROM RBL_MISDB_PROD.Automate_Advances 
          WHERE  EXT_FLG = 'Y';
         UPDATE tt_AccountCal_23
            SET TOTALPROVISION = 0,
                BANKTOTALPROVISION = 0,
                RBITOTALPROVISION = 0;
         UPDATE tt_CUSTOMERCAL_36
            SET TOTPROVISION = 0,
                BANKTOTPROVISION = 0,
                RBITOTPROVISION = 0;
         ...
         , NonFinDPD, InitialAssetClassAlt_Key, FinalAssetClassAlt_Key, RestructureProvision, SecuredProvision, UnSecuredProvision, FlgDeg, FlgUpg, DegDate, RC_Pending, FinalNpaDt, RestructureStage, EffectiveFromTimeKey, EffectiveToTimeKey, FlgMorat, InvestmentGrade, POS_10PerPaidDate, RestructureFacilityTypeAlt_Key )
           ( SELECT
             ...
             FROM PRO_RBL_MISDB_PROD.AdvAcRestructureCal_Hist a
                    JOIN tt_AccountCal_23 B   ON A.AccountEntityId = B.AccountEntityID
         ...
         UPDATE tt_AccountCal_23
          ...
          WHERE  NVL(TOTALPROVISION, 0) > NetBalance
           AND NVL(NetBalance, 0) > 0;
         IF utils.object_id('TEMPDB..tt_TOTALPROVCUST_4') IS NOT NULL THEN
          EXECUTE IMMEDIATE ' TRUNCATE TABLE tt_TOTALPROVCUST_4 ';
         END IF;
         DELETE FROM tt_TOTALPROVCUST_4;
         UTILS.IDENTITY_RESET('tt_TOTALPROVCUST_4');

         INSERT INTO tt_TOTALPROVCUST_4 ( 
         ...
         FROM A ,tt_CUSTOMERCAL_36 A
                JOIN tt_TOTALPROVCUST_4 B   ON A.CUSTOMERENTITYID = B.CUSTOMERENTITYID ) src
         ON ( A.ROWID = src.row_id )
