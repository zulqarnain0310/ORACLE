...
BEGIN
   ...
   IF tt_CustomerCal_BACKDTD  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_CustomerCal_BACKDTD;
   UTILS.IDENTITY_RESET('tt_CustomerCal_BACKDTD');

   INSERT INTO tt_CustomerCal_BACKDTD ( 
   	SELECT A.* ,
   ...
   MERGE INTO TEMP 
   USING (SELECT TEMP.ROWID row_id, ACCT.EntityKeyNew
   FROM TEMP ,tt_CustomerCal_BACKDTD TEMP
          JOIN ( SELECT tt_CustomerCal_BACKDTD.CustomerEntityId ,
                        (v_EntityKeyCust1 + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                             FROM DUAL  )  )) EntityKeyNew  
                 FROM tt_CustomerCal_BACKDTD ---Where IsChanged in ('C','N')
                        ) ACCT   ON TEMP.CustomerEntityId = ACCT.CustomerEntityId ) src
   ON ( TEMP.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET TEMP.EntityKeyNew = src.EntityKeyNew;
   --WHERE Temp.IsChanged in ('C','N')
   INSERT INTO PRO_RBL_MISDB_PROD.CustomerCal_Hist
     ...
     ( SELECT T.EntityKeyNew ,
              ...
              NULL ChangeFld  ,
              NULL ScreenFlag  
       FROM tt_CustomerCal_BACKDTD T
        WHERE  EffectiveToTimeKey > v_TIMEKEY );
   /* ACCOUNT - BACKDTD */
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_AccountCal_BACKDTD  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_AccountCal_BACKDTD;
   UTILS.IDENTITY_RESET('tt_AccountCal_BACKDTD');

   INSERT INTO tt_AccountCal_BACKDTD ( 
   	SELECT A.* ,
           UTILS.CONVERT_TO_NUMBER(0,19,0) EntityKeyNew  
   ...
   MERGE INTO TEMP 
   USING (SELECT TEMP.ROWID row_id, ACCT.EntityKeyNew
   FROM TEMP ,tt_AccountCal_BACKDTD TEMP
          JOIN ( SELECT tt_AccountCal_BACKDTD.AccountEntityID ,
                        (v_EntityKeyAcct1 + ROW_NUMBER() OVER ( ORDER BY ( SELECT 1 
                                                                             FROM DUAL  )  )) EntityKeyNew  
                 FROM tt_AccountCal_BACKDTD ---Where IsChanged in ('C','N')
                        ) ACCT   ON TEMP.AccountEntityID = ACCT.AccountEntityID ) src
   ON ( TEMP.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET TEMP.EntityKeyNew = src.EntityKeyNew;
   --WHERE Temp.IsChanged in ('C','N')
