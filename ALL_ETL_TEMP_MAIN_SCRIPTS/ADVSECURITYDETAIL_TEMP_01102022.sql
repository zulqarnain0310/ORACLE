--------------------------------------------------------
--  DDL for Procedure ADVSECURITYDETAIL_TEMP_01102022
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" 
-- =============================================  
 -- Author:  <Author,,Name>  
...
AS
   -- -- SET NOCOUNT ON added to prevent extra result sets from  [RBL_MISDB_010922_UAT]
...
BEGIN
   ...
   IF tt_RBL_SECURITY_CAL_2  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_RBL_SECURITY_CAL_2;
   UTILS.IDENTITY_RESET('tt_RBL_SECURITY_CAL_2');

   INSERT INTO tt_RBL_SECURITY_CAL_2 ( 
   	SELECT * ,
   ...
   FROM A ,tt_RBL_SECURITY_CAL_2 A
          JOIN ( SELECT tt_RBL_SECURITY_CAL_2.CollateralID ,
                        COUNT(DISTINCT tt_RBL_SECURITY_CAL_2.CUSTOMER_ID)  COUNTS  
                 FROM tt_RBL_SECURITY_CAL_2 
                  WHERE  tt_RBL_SECURITY_CAL_2.COLLATERAL_TYPE = 'Immovable Property'
                           AND tt_RBL_SECURITY_CAL_2.ValuationExpiryDate > v_date
                   GROUP BY tt_RBL_SECURITY_CAL_2.CollateralID ) B   ON A.CollateralID = B.CollateralID ) src
   ...
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_CALC_2  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_CALC_2;
   UTILS.IDENTITY_RESET('tt_CALC_2');

   INSERT INTO tt_CALC_2 ( 
   	SELECT CUSTOMER_ID ,
           A.CollateralID ,
   ...
   WITH CTE_A AS ( SELECT CUSTOMER_ID ,
    ...
    WHERE  COLLATERAL_TYPE = 'Immovable Property'
      ...
      FROM A ,CTE_A A
             JOIN tt_CALC_2 B   ON A.Customer_ID = B.Customer_ID
   ...
   WITH CTE_A AS ( SELECT CUSTOMER_ID ,
    ...
    WHERE  ValuationExpiryDate > v_date
             AND COLLATERAL_TYPE IN ( 'Insurance/NSC/KVP/IVP','Shares/MF/Gold','Movable FA' )
    ),
   ...
   /* CLEANED DATA OUTPUT */
   /* STOCK DATA WORK */
   --select *  from tt_RBL_SECURITY_CAL_2_STOCK where SecValue_Final>0 and ValuationExpiryDate>='2022-03-19'
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_RBL_SECURITY_CAL_2_STOCK  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_RBL_SECURITY_CAL_STOCK_2;
   UTILS.IDENTITY_RESET('tt_RBL_SECURITY_CAL_STOCK_2');

   INSERT INTO tt_RBL_SECURITY_CAL_STOCK_2 ( 
   	SELECT * ,
   ...
   UPDATE tt_RBL_SECURITY_CAL_STOCK_2
      SET SecValue_Final = 0;
   ---select *  from tt_RBL_SECURITY_CAL_2_STOCK   where stock_audit_date
   ---		select * from tt_RBL_SECURITY_CAL_2_STOCK where Cust_OS<50000000 and ValuationExpiryDate>=@Date
   WITH CUST_OS AS ( SELECT A.RefCustomerId ,
     ...
     GROUP BY A.RefCustomerId ) 
      ...
      FROM A ,tt_RBL_SECURITY_CAL_STOCK_2 A
   ...
   --DELETE tt_RBL_SECURITY_CAL_2_STOCK WHERE stock_audit_date IS NULL and ISNULL(CUST_OS,0)>=50000000
   ...
   ---DELETE tt_RBL_SECURITY_CAL_2_STOCK WHERE  (ValuationExpiryDate_new<'2022-03-22' )
   --or ValuationExpiryDate_new is null )
   ----		SELECT * FROM tt_RBL_SECURITY_CAL_2_STOCK WHERE Customer_ID ='2098381'
   WITH CTE_A AS ( SELECT CUSTOMER_ID ,
                          SecValue_Final ,
                          CollateralID ,
                          ROW_NUMBER() OVER ( PARTITION BY CUSTOMER_ID, CollateralID ORDER BY Customer_ID, CollateralID, SecurityValue  ) RID  
     FROM tt_RBL_SECURITY_CAL_STOCK_2 
    WHERE  ( NVL(CUST_OS, 0) >= 50000000
      ...
      ;
   ----ALTER TABLE tt_RBL_SECURITY_CAL_2 add ValuationExpiryDate_new date
   UPDATE tt_RBL_SECURITY_CAL_2
      SET SecValue_Final = 0
    WHERE  ( ValuationExpiryDate <= v_Date
     OR ( SecurityValue = 0
   ...
   INSERT INTO RBL_STGDB.FINACLE_COLLATERAL_DATA
     ( CUSTOMER_ID, COLLATERALID, SECURITYCODE, VALUATIONDATE, ValuationExpiryDate_new, MainSecurityValue, SecValue_Final, CustomerACID, COLLATERAL_TYPE )
     ( SELECT CUSTOMER_ID ,
              COLLATERALID ,
              SECURITYCODE ,
              VALUATIONDATE ,
       ...
       FROM ( SELECT CUSTOMER_ID ,
              ...
              SELECT CUSTOMER_ID ,
                     ...
                     SecValue_Final ,
                     CASE 
                          WHEN COLLATERAL_TYPE = 'Deposits' THEN AccountID
                     ELSE NULL
                        END customeracid  ,
                     COLLATERAL_TYPE 
              FROM tt_RBL_SECURITY_CAL_2 
         ...
         GROUP BY CUSTOMER_ID,COLLATERALID,SECURITYCODE,VALUATIONDATE,ValuationExpiryDate_new,Customeracid,COLLATERAL_TYPE );
   --alter table RBL_STGDB.DBO.FINACLE_COLLATERAL_DATA
   --add COLLATERAL_TYPE varchar(100)
   --select * from tt_RBL_SECURITY_CAL_2
   --select * from  tt_RBL_SECURITY_CAL_2_STOCK where ValuationExpiryDate_new<=@Date
   ...
   INSERT INTO TempAdvSecurityDetail
     ...
     ( 
       ...
       FROM RBL_STGDB.FINACLE_COLLATERAL_DATA A
              ...
              AND B.EffectiveFromTimeKey <= v_TimeKey
              AND B.EffectiveToTimeKey >= v_TimeKey
        WHERE  NVL(A.COLLATERAL_TYPE, ' ') <> 'Deposits'
       UNION 
       ---------------------------------------------------------------
       ALL 
...
/
  ...
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "MAIN_PRO";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "MAIN_PRO";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "RBL_TEMPDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_01102022" TO "ADF_CDR_RBL_STGDB";
