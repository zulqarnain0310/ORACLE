--------------------------------------------------------
--  DDL for Procedure ADVSECURITYDETAIL_TEMP_07102022_1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" 
-- =============================================  
 -- Author:  <Author,,Name>  
...
BEGIN
   ...
   SELECT TimeKey - 1 
     ...
     FROM RBL_MISDB_010922_UAT.Automate_Advances 
    WHERE  EXT_FLG = 'Y';
   UPDATE RBL_MISDB_010922_UAT.AdvSecurityDetail
      SET EffectiveToTimeKey = v_PrvTimeKey
    WHERE  EffectiveToTimeKey = 49999;
   UPDATE RBL_MISDB_010922_UAT.AdvSecurityValueDetail
      SET EffectiveToTimeKey = v_PrvTimeKey
    WHERE  EffectiveToTimeKey = 49999;
   ---------------------------------------------------------------------------------------------------------------------------------------------------   
   /* FINACLE COLLATEAL WORK*/
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_RBL_SECURITY_CAL_4  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_RBL_SECURITY_CAL_4;
   UTILS.IDENTITY_RESET('tt_RBL_SECURITY_CAL_4');

   INSERT INTO tt_RBL_SECURITY_CAL_4 ( 
   	SELECT * ,
   ...
   FROM A ,tt_RBL_SECURITY_CAL_4 A
          JOIN ( SELECT tt_RBL_SECURITY_CAL_4.CollateralID ,
                        COUNT(DISTINCT tt_RBL_SECURITY_CAL_4.CUSTOMER_ID)  COUNTS  
                 FROM tt_RBL_SECURITY_CAL_4 
                  WHERE  tt_RBL_SECURITY_CAL_4.COLLATERAL_TYPE = 'Immovable Property'
                           AND tt_RBL_SECURITY_CAL_4.ValuationExpiryDate > v_date
                   GROUP BY tt_RBL_SECURITY_CAL_4.CollateralID ) B   ON A.CollateralID = B.CollateralID ) src
   ...
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_CALC_4  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_CALC_4;
   UTILS.IDENTITY_RESET('tt_CALC_4');

   INSERT INTO tt_CALC_4 ( 
   	SELECT CUSTOMER_ID ,
           A.CollateralID ,
   ...
   WITH CTE_A AS ( SELECT CUSTOMER_ID ,
    ...
    WHERE  COLLATERAL_TYPE = 'Immovable Property'
      ...
      FROM A ,CTE_A A
             JOIN tt_CALC_4 B   ON A.Customer_ID = B.Customer_ID
   ...
   /* CLEANED DATA OUTPUT */
   /* STOCK DATA WORK */
   --select *  from tt_RBL_SECURITY_CAL_4_STOCK where SecValue_Final>0 and ValuationExpiryDate>='2022-03-19'
   IF  --SQLDEV: NOT RECOGNIZED
   IF tt_RBL_SECURITY_CAL_4_STOCK  --SQLDEV: NOT RECOGNIZED
   DELETE FROM tt_RBL_SECURITY_CAL_STOCK_4;
   UTILS.IDENTITY_RESET('tt_RBL_SECURITY_CAL_STOCK_4');

   INSERT INTO tt_RBL_SECURITY_CAL_STOCK_4 ( 
   	SELECT * ,
   ...
   UPDATE tt_RBL_SECURITY_CAL_STOCK_4
      SET SecValue_Final = 0;
   ---select *  from tt_RBL_SECURITY_CAL_4_STOCK   where stock_audit_date
   ---		select * from tt_RBL_SECURITY_CAL_4_STOCK where Cust_OS<50000000 and ValuationExpiryDate>=@Date
   WITH CUST_OS AS ( SELECT A.RefCustomerId ,
     ...
     GROUP BY A.RefCustomerId ) 
      ...
      FROM A ,tt_RBL_SECURITY_CAL_STOCK_4 A
   ...
   --DELETE tt_RBL_SECURITY_CAL_4_STOCK WHERE stock_audit_date IS NULL and ISNULL(CUST_OS,0)>=50000000
   ...
   ---DELETE tt_RBL_SECURITY_CAL_4_STOCK WHERE  (ValuationExpiryDate_new<'2022-03-22' )
   --or ValuationExpiryDate_new is null )
   ----		SELECT * FROM tt_RBL_SECURITY_CAL_4_STOCK WHERE Customer_ID ='2098381'
   WITH CTE_A AS ( SELECT CUSTOMER_ID ,
                          SecValue_Final ,
                          CollateralID ,
                          ROW_NUMBER() OVER ( PARTITION BY CUSTOMER_ID, CollateralID ORDER BY Customer_ID, CollateralID, SecurityValue  ) RID  
     FROM tt_RBL_SECURITY_CAL_STOCK_4 
    WHERE  ( NVL(CUST_OS, 0) >= 50000000
      ...
      ;
   ----ALTER TABLE tt_RBL_SECURITY_CAL_4 add ValuationExpiryDate_new date
   UPDATE tt_RBL_SECURITY_CAL_4
      SET SecValue_Final = 0
    WHERE  ( ValuationExpiryDate <= v_Date
     OR ( SecurityValue = 0
   ...
   --select * from tt_RBL_SECURITY_CAL_4
   --select * from  tt_RBL_SECURITY_CAL_4_STOCK where ValuationExpiryDate_new<=@Date
   ...
   ON ( TEMP.ROWID = src.row_id )
   WHEN MATCHED THEN UPDATE SET TEMP.SecurityEntityId = src.SecurityEntityId;/*
   --=============================================Added by Prashant as per Amar sir 02-10-2022============================================
   ...
   select * into #temp2
   from (
   select count(*)Cnt, SecurityEntityID from RBL_TEMPDB.DBO.[TempAdvSecurityDetail]
   group by SecurityEntityID
   having count(*)>1
   )A

   Update        a
   set           a.SecurityEntityID=0
    from         RBL_TEMPDB.DBO.[TempAdvSecurityDetail] a
   inner join    #temp2 b
   on            a.SecurityEntityID=b.SecurityEntityID

   --  New Customers Account Entity ID Update  --
   DECLARE @SecurityEntityId1 INT=0 
   SELECT @SecurityEntityId1=MAX(SecurityEntityId) FROM  RBL_TEMPDB.DBO.[TempAdvSecurityDetail]
   ...
   SET TEMP.SecurityEntityId=ACCT.SecurityEntityId
    FROM RBL_TEMPDB.DBO.[TempAdvSecurityDetail] TEMP
   INNER JOIN (SELECT CustomerEntityId,AccountEntityId,CollateralId,SecurityType,(@SecurityEntityId1 + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) SecurityEntityId
   			FROM RBL_TEMPDB.DBO.[TempAdvSecurityDetail]
   			WHERE SecurityEntityId=0 OR SecurityEntityId IS NULL)
   		ACCT 
   	ON 	 TEMP.CollateralId		=ACCT.CollateralId	
...
/
  ...
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ROLE_ALL_DB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "CC_CDR_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "MAIN_PRO";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_BI_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ALERT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ACL_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "QPI_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "CURDAT_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "PREMOC_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "BSG_READ_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "STD_FIN_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "BS_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "DWH_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_TEMPDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "STG_FIN_RBL_STGDB";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT EXECUTE ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ADF_CDR_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ROLE_ALL_DB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "CC_CDR_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "MAIN_PRO";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_BI_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ALERT_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ACL_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "D2KMNTR_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "QPI_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "CURDAT_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "PREMOC_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "BSG_READ_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "STD_FIN_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "BS_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "DWH_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "RBL_TEMPDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ETL_MAIN_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "STG_FIN_RBL_STGDB";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "DATAUPLOAD_RBL_MISDB_PROD";
  GRANT DEBUG ON "ETL_TEMP_RBL_TEMPDB"."ADVSECURITYDETAIL_TEMP_07102022_1" TO "ADF_CDR_RBL_STGDB";
